...
\delta\left(h^{\lam}_i,
\cala_i^\lam(\sum_{j=0}^{nh(\lam-1)-1}
 w^{\lam}_{i|j}
...
Make the nodes $\rvh_0^\lam$
for all $\lam$ and $\rvY_0$
root nodes (so no arrows pointing into them).
...
above steps,
the transition prob matrices,
printed in blue,
for the nodes of the NN bnet are as follows.
...
and define the transition prob matrix
