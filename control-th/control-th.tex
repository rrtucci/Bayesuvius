...
where
\beq
\caln(s)= \prod_{a=1}^{A}(s-s_a)^{na}
\eeq
and

\beq
\cald(s)= \prod_{b=1}^{B}(s-s_b)^{nb}
\eeq
and where 
the polynomials $\caln(s)$ and $\cald(s)$
have no common factors. The $\{s_a\}_{\forall a}$ are
called the {\bf zeros}
of the transfer function,
and the $\{s_b\}_{\forall b}$ are called the
...
\item {\bf unstable}
if $Re(s_b)> 0$ for some $b$.
\end{itemize}

To get an intuition for why this is so,
...
where $\gamma, T\in\RR$
and $Re(s_b)< \gamma$.
At the pole, $s=s_b=\s_b+i\omega_b$, so


\beq
e^{st}=e^{s_b t} = e^{\s_b t} 
e^{i\omega_b t}
\eeq
As $t\rarrow \infty$, 
