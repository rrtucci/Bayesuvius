...
A linear system with input $u(t)$
and output $y(t)$
has  a kernel function $H(t, \tau)$
such that

\beq
y(t)=\int_0^{\Lambda} H(t,\tau)u(\tau)
\eeq
In general,
$u$ and $y$
are vectors, so
...
An system is {\bf stable}
if a bounded input $u(\cdot)$  implies
