...
{\bf transition function}. It maps each place node $\rvp(t)\in\calp$ and transition $\rvx(t)\in\calx$ to the next place node $\rvp(t+1)\in\calp$ in a {\bf place history} $\rvp(0), \rvp(1), \ldots$. We will write this as
...
transfers the node accross without adding anything to the string history. 
\item
we will often indicate a node for 
