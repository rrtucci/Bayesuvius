...
$P(\rvtau\geq t\mid \text{patient dies rather than being censored})$})
\beqa
\HAT{S}(\tau^j) &=&
...
\\
&=&
\prod_{k\leq j}P\left(\rvo(\tau^k)=0\mid \bigA_{k'<k}\{\rvo(\tau^{k'})=0\}\right)
\quad \text{(chain rule)}
\\
&=&
\left\{
\begin{array}{l}
\prod_{k\leq j}P\left(\rvd(\tau^k)=0\mid \bigA_{k'<k}\{\rvo(\tau^{k'})=0\}\right)
\\
+
\underbrace{
\prod_{k\leq j}P\left(\rvc(\tau^k)=0\mid \bigA_{k'<k}\{\rvo(\tau^{k'})=0\}\right)
}_{=0 \text{(see footnote)}}
\end{array}\right.
\\
&=&
\prod_{k\leq j}\left[
1-P\left(\rvd(\tau^k)=1\mid \bigA_{k'<k}\{\rvo(\tau^{k'})=0\}\right)\right]
\\
&=&
...
\beq \color{blue}
P(\rvo(\tau^k)=1\mid \bigA_{k'<k}\{\rvo(\tau^{k'})=0\})
= \frac{n_O^k}{n_R^k}
\eeq

\beq \color{blue}
P(\rvd(\tau^k)=1\mid \rvo(\tau^{k})=1)
= \frac{n_D^k}{n_O^k}
\eeq

\beq \color{blue}
P(\rvc(\tau^k)=1\mid \rvo(\tau^{k})=1)
= \frac{n_C^k}{n_O^k}
\eeq
...
=
P(\rvtau^k=\tau^j\mid \rvtau^k\geq \tau^j)
\eeq
Therefore
...
\begin{align}
 L^j (\beta)&=
\frac{ P(Z^j\mid\tau^{j})}
{\sum_{k\geq j}P(Z^{k}\mid\tau^{j})}
\\
&=
\frac{ P(Z^j\mid\tau^{j})}
{P(\bigV_{k\geq j}\{\ul{Z}^{k}=Z^k\}\mid\tau^{j})}
...
\\
&=
P(Z^j\mid \bigV_{k\geq j}\{\ul{Z}^{k}=Z^k\},\tau^{j})
\end{align}
...
\\
 P(\rvtau=\tau^j)=
 \lam(\tau^j\mid Z^j) S(\tau^j)
  & \text{ if $d(\tau^j)=1$ }
 \end{array}\right.
 \\
 &=
 \lam(\tau^j\mid Z^j)^{d(\tau^j)} S(\tau^j)
 \\
 &=
\underbrace{\left[\frac{\lam(\tau^j\mid Z^j)}
{\sum_{k\geq j} \lam(\tau^k\mid Z^j)}
\right]^{d(\tau^j)}
}_{L_1}
\underbrace{
\left[
...
\right]
\text{(because $e^x\approx 1 + x$ for $|x|\ll 1$)}
\end{align}
