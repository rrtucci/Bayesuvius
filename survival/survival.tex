...
Note that we define in this chapter
 $S(t) = P(\rvtau\geq t)$ (present among survivors (PAS))
...
Kaplan-Meier (KM) estimator of $S(t)$ (see\footnote{Even though $S(t)$ has
...
In a {\bf Cox time-dependent-hazards (TDH) model},
...
of indices $j$ of $\tau^j$. So we can label the population individuals by
the index $j(\s)$.
