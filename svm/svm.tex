...
Define the {\bf Cost of erring} to be
\beq
CE(x^\s, y^\s)
=
(1-y^\s Y(x^\s))_+
\eeq
$CE$ is shown in Fig.\ref{fig-svm-hinge}.
As you can see, 
...
should have the lowest $CE$
of all possible $D_{DMZ}$. So to find
that line of demarcation,
we want to minimize  $CE$
with respect to $D_{DMZ}$.

Note also that $CE$ starts at zero
and increases monotonically
as $D_{DMZ}$ grows.
because when $DMZ$ grows,
more
$x^\s$ live
inside the DMZ and
...
\beqa
\call&=& \frac{2}{D_{DMZ}^2} + \sum_\s CE(x^\s, y^\s)
\\
&=&
