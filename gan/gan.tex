...
\beq\color{blue}
P(f[i]\mid \vecu, \theta_G)= \delta[f[i], G(u[i],\theta_G)]
\eeq

\beq\color{blue}
P(c[i]\mid \vecf, \theta_D) = \delta(c[i], \ol{D}(f[i], \theta_D))
\eeq

\beq\color{blue}
P(d[j]\mid \vecr, \theta_D)= \delta(d[j], D(r[j], \theta_D))
\eeq
...
\beq\color{blue}
P(R[i]\mid \theta_G, \theta_D)= \frac{D(R[i], \theta_D)}{\lam(\theta_D)}
\eeq
...
P(\ul{R[i]}=r[i]\mid \theta_G, \theta_D)P(\ul{U[i]}=u[j]\mid \theta_G)))
\eeq
where $N=nsam(\vecr)nsam(\vecu)$.
