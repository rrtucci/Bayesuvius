...
were also
helpful in understanding the subject.


How things stand
in bnet learning software reminds me
of how things stand in linear 
...
\begin{algorithm}
    ...
    \KwOut{a bnet $B=(G, T)$,
	where $G=(V, E)$ is a DAG, and $E$ are the edges (arrows).
 	$T$ are Transition Probability Matrices (TPMs)
 	$T=TPMs(G, D)$. }
...
\begin{algorithm}
	...
	acyclic graph $G=(V, E, EU)$, where $V$ are 
	the vertices (nodes),
	...
	\rvx\neq \rvy\}$\;
	\tcp{Shrink phase. Deletes edges.}
	\For{all $\rvx-\rvy\in UE$}{
		$info\_transmission\larrow True$\;
		\For{all $S\subset V-\{\rvx, \rvy\}$}{
			\If{$H(\rvx:\rvy|S)<\eps$}{
				$info\_transmission \larrow
 				False$\;
				break out of loop\;
			}
		}
		\If{$!info\_transmission$}{
			$UE\larrow UE-\{\rvx-\rvy\}$\;
		}
	}
	\tcp{Growth phase. Adds v structures.}
	\For{$\rvx,\rvy, \rva$ such that
	$\rvx-\rva\in UE, \rva-\rvy\in UE,
	\rvx-\rvy\not \in uE$}{
		$info\_transmission \larrow False$\;
		\For{$S\subset V-\{\rvx, \rvy,\rva\}$}{
			\If{$H(\rvx:\rvy|S\cup\{\rva\})>\eps$}{
				$info\_transmission \larrow True$\;
				break out of loop\;
			}
		}
		\If{$info\_transmission$}{
			$UE\larrow E-\{\rvx-\rva, \rva-\rvy\}$\;
    ...
    \KwRet{$G=(V, E, UE)$}
    \caption{Pseudo-code for Grow Shrink algorithm}
\end{algorithm}
