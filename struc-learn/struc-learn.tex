...
\begin{algorithm}
	...
	\For{$\lam=0, 1, 2, \ldots, |V|-2$}{
		\For{all $\rvx_i-\rvx_j\in UE$}{
			\For{all $S=\{\rvx_k\in V:\rvx_i-\rvx_k\in UE, 
			k\neq j, i\} \ni |S|=\lam$}{
				\If{$H(\rvx_i:\rvx_j|S)<\eps$}{
					\tcc{If there were an arrow between $\rvx_i$ and $\rvx_j$, 
						then conditioning on $S$ would not be enough
						to interrupt info transmission $H(\rvx_i:\rvx_j|S)$
						 between $\rvx_i$ and $\rvx_j$}
					$UE\larrow UE-\{\rvx_i-\rvx_j\}$\;
					$S(\rvx_i-\rvx_j)\larrow S$\;
				}
			}
		}
	}
	...
	\rvx-\rvy\not \in UE, \rva\not\in S(\rvx_i-\rvx_j)$}{
		...
		there would be info transmission  between $\rvx_i$ and
		$\rvx_j$}
