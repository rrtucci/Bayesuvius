...
\section{redoing claim}

\begin{claim}\label{cl-de-diriclet-green}
The solution to the equation
\beq
\call \eta \oneevent{x}{t} = S\oneevent{x}{t}\quad
\eeq
for $x>0$ and $t>0$
with initial conditions
\beq
\eta\oneevent{x}{0}= \eta_\rvx(x)
\eeq
and Dirichlet boundary conditions:

\beq
\eta\oneevent{0}{t} = \eta_\rvt(t)
\eeq
is
...
&&-D
\int_0^{t} dt'\;
\left[ G\twoevent{x}{t}{x'}{t'}\cev{\partial}_{x'}\right]_{x'=0}
 \eta_\rvt(t')
\eeqa
where $G\twoevent{x}{t}{x'}{t'}$ satisfies
the following equations for $x>0$ and $t>0$
...
\beq
\call^\dagger_{x,t}= -\cev{\partial}_t -D \cev{\partial}_x^2,
\quad G^\dagger\twoevent{x'}{t'}{x}{t}\call^\dagger_{x,t}=\stackt{\delta(x-x')}{\delta(t-t')}
\eeq
...
Define $I$, $I_\rvx$ and $I_\rvt$  by
\beq
I = G\twoevent{x}{t}{x'}{t'}\call_{x',t'}\eta\oneevent{x'}{t'}
-
\eta\oneevent{x'}{t'}\call^\dagger_{x',t'}
G^\dagger\twoevent{x'}{t'}{x}{t}
\eeq
...
}=\brat{x}{t}\rv{\call}^\dagger G^\dagger\kett{x'}{t'}=
...
\beq
\bra{t}G\ket{t'}= \bra{t'}G^\dagger\ket{t}
\eeq
