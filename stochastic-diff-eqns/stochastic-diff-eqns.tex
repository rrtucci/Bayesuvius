...
if $R_{\mu\nu}=\delta_{\mu\nu} \frac{1}{2\lam}>0$, then the steady state
solution is
...
$\partial_{x_i}$ (not the $\partial_t$'s) is
...
\
The {\bf forward FP equation} for transition matrix $ P(\XT{x}{t}|\XT{w}{s})$
is  defined as
...
The {\bf backward FP equation} for transition matrix $P(\XT{y}{u}|\XT{x}{t})$
is defined as
...
\beq
\frac{d}{dt}\phi(\XT{x}{t})= \Omega_{x,t}\phi(\XT{x}{t})
\to \quad 
\frac{d}{dt} \av{x|\phi_t}= \Omega_{x,t}\av{x|\phi_t}
\to\quad
\frac{{d}}{dt}\ket{\phi_t}=
\Omega_{\rvx,t}\ket{\phi_t}
\label{eq-pde-dirac-notation}
\eeq
and an expectation value in Dirac notation\footnote{Note that
we don't use a complete
set $\ket{\XT{x}{t}}$
because the expectation
value sums over all $x$ values only,
not over all $x$ and $t$ values.}
...
\to \quad
E_{\rvx}[A(\rvx, t)]=
\int dx\;\av{P|x}\av{x|A_t}=
\av{P|A_t}
\eeq
Now applying $E_\rvx[\cdot]$ to
both sides of Eq.(\ref{eq-pde-dirac-notation}), we get

\beq
\av{P|\frac{d}{dt} |\phi_t}=
\av{P|\Omega_{\rvx,t}|\phi_t}
\eeq
Now taking the Hermitian conjugate of both sides with respect to
$x$ only (not $t$)

\beq
\frac{d}{dt}\av{\phi_t|P}=
\av{\phi_t|\Omega^\dagger_{\rvx,t}|P}
\eeq
Setting $\bra{\phi_t}=\bra{x_t}$ we get
\beq
\frac{d}{dt}\av{x_t|P}=
\av{x_t|\Omega^\dagger_{\rvx,t}|P}
\eeq
...
\beq
\frac{d}{dt}\av{x_t| P}=
\Omega^\dagger_{x,t}\av{x_t|P}
\eeq
so
\beq
\frac{d}{dt}\av{x_t| P}=
\calf_{x,t}\av{x_t|P},\quad
...
\beq
\frac{d}{dt}
\underbrace{\av{x_t| P|\XT{w}{s}}}_
{P(\XT{x}{t}|\XT{w}{s})}=
\calf_{x,t}\av{x_t|P|\XT{w}{s}},\quad
\calf_{x,t}=\Omega^\dagger_{x,t}
\eeq
...
\beq
\frac{d}{dt}
\underbrace{\av{\XT{y}{u}| P|x_t}}_{
P(\XT{y}{u}|\XT{x}{t})}=
\calb_{x,t}\av{\XT{y}{u}|P|x_t},\quad
...
true backwards prediction would entail
a PDE for $\frac{d}{dt} [P(\XT{y}{u}|\XT{x}{t})P(\XT{x}{t})]$,
as is done for Doob's $h$-transform
(described later).
