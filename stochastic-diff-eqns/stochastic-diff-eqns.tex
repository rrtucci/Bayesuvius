...
&=&
\left[\pder{\phi}{t} +
f_\mu \pder{\phi}{x_\mu} + \frac{R_{\mu, \nu}}{2}
\frac{\partial^2\phi}{\partial x_\mu\partial x_\nu}\right]dt
+
\pder{\phi}{x_\mu}L_{\mu,\nu}d\rvB_\nu
\eeqa
...
-\;\pder{}{x_\mu}
(\bullet f_\mu) + 
\frac{\partial^2}{ \partial x_\nu\partial x_\mu}(\bullet \frac{R_{\mu, \nu}}{2})
\eeq
\end{claim}
\proof
...
\\
&=&
(F-i\omega)^{-1}\underbrace{LQL^\dagger}_R(F+i\omega)
\eeqa

\beq
C_{\rvx}(\tau)= \frac{1}{2\pi}
...
\beq
P^D(y_{t+s}|x_t)=\frac{D(.|y_{t+s})P(y_{t+s}|x_t)}
{D(.|x_t)}
\eeq
...
If
\beq
\left[\pder{}{s} + \calb_\rvy\right] \phi(y_s)=0,
\eeq
...
\beq
\begin{array}{l}
\left[\pder{}{s} + \calb_\rvy\right] D(.|y_s)=0,
\\
(dy_\mu = f_\mu(y_s)ds + L_{\mu,\nu}(y_s)d\rvB_\nu)
\end{array}
\eeq
...
\beq
P(A|B, x_t)=\frac{P(B|A, x_t)P(A|x_t)}
{P(B|x_t)}
\eeq
