...
$\rvx(t_{j\ldots k}) =\{  x(\tau): \tau\in\{t_j, t_{j+1}, \ldots, t_k\}\}$
...
(discussed later)
aren't (because they are semi-martingales with a drift term).
...
\eeq
For a {\bf Markov chain}

\beq
P(x(t)| x([0, s]))=
P(x(t)| x(s))
\eeq
Any process $\rvx_t$ satisfies the
