...
its set of d-separations by
\beq
DS(G)=\{\rvA.\perp_G\rvB.|\rvZ.:
\text{ $\rvA.,\rvB.,\rvZ.$ are multinodes of $G$\}}
\;.
\eeq
Given a probability distribution  $P$, define 
its set of conditional independencies by
\beq
CI(P)=\{\rvA.\perp_P\rvB.|\rvZ.:
...
distribution $P$
can be compatible with several $G$.
For example, $P$
might be compatible with
the fully connected
DAG and with a DAG that
has fewer edges than the 
fully connected DAG.
...
by
\beqa
\phi:DS(G) &\rarrow& CI(P)
\\
\phi: \rvA.\perp_G\rvB.|\rvZ.&\mapsto&
\rvA.\perp_P\rvB.|\rvZ.
\eeqa
