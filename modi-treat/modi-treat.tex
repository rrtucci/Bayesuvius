...
\beq
P(\vecy|\vtheta, \phi) =
\exp\left(\frac{\vtheta\cdot \vecy - b(\vtheta)}{a(\phi)}+c(y,\phi)\right)
\eeq
...
\beq
\av{\rvy_\s, \rvy_{\s'}} = a\partial_{\theta_\s}\partial_{\theta_{\s'}}b
\eeq
...
&=&
\exp\left( y_\s
\underbrace{\ln\left(\frac{p_\s}{1-p_\s}\right)}_{\theta_\s}
...
&=&
-\ln\left(
1- \frac{1}{1+e^{-\theta_\s}}
\right)
\\
&=&
-\ln\left(
\frac{e^{-\theta_\s}}{1+e^{-\theta_\s}}
\right)
\\
&=&
\ln(1+e^{\theta_\s})
...
$\theta_\s=g(\mu_\s)$ & \cellcolor[HTML]{F6F694}$\mu_\s=g^{-1}(\theta_\s)$ \\ \hline
\begin{tabular}[c]{@{}l@{}}Normal\\ $\rvy_\s\in(-\infty, +\infty)$\end{tabular} & \begin{tabular}[c]{@{}l@{}}$\theta_\s= \mu_\s$\\ ($g$= identity map)\end{tabular} & $\mu_\s = \theta_\s$ \\ \hline
\begin{tabular}[c]{@{}l@{}}Exponential\\ $\rvy_\s\in  (0, +\infty)$\end{tabular} & $\theta_\s= -\frac{1}{\mu_\s}$ & $\mu _\s= - \frac{1}{\theta_\s}$ \\ \hline
\begin{tabular}[c]{@{}l@{}}Poisson\\ $\rvy_\s\in \{0,1,2, \ldots\}$\end{tabular} & $\theta_\s = \ln \mu_\s$ & $\mu_\s = \exp(\theta_\s)$ \\ \hline
\begin{tabular}[c]{@{}l@{}}Bernoulli\\ $\rvy_\s\in\bool$\end{tabular} & $\theta_\s = \lodds(\mu_\s)$ & $\mu_\s = \smoid(\theta_\s)$ \\ \hline
\end{tabular}
...
\beq
\mu_\s = \partial_{\theta_\s} b
\eeq

\beq
X_\s^T d\beta= d\mu_\s = (\partial^2_{\theta_\s} b)d\theta_\s
\eeq

\beq
X_\s WX_\s^T d\beta = (\partial^2_{\theta_\s} b)  X_\s W   d\theta_\s
\eeq

\beq
\der{\beta}{\theta_\s}=  (\partial^2_{\theta_\s} b) (X_\s WX_\s^T)^{-1}  X_\s W
\eeq
