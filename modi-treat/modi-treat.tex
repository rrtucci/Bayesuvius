...
\beq
\TIL{\caly}_{|x, c} =
\sum_\tilx\sum_y y P(\rvy(\tilx)=y\cond x, c)
P(\tilx|x,c)
\eeq

If $P(\tilx|x,c)=\delta(\tilx, \tilx_c(x))$,
then
\beqa
\TIL{\caly}_{|x, c} &=&
\sum_\tilx\sum_y y P(\rvy(\tilx)=y\cond \rvx=x, c)
\delta(\tilx, \tilx_c(x))
\\
&=&
\sum_\tilx\sum_y y P(\rvy(\tilx_c(x))=y\cond \ul{\tilx}=\tilx_c(x), c)
\delta(\tilx, \tilx_c(x))
\\
&=&
\sum_\tilx\sum_y y P(\rvy=y\cond \ul{\tilx}=\tilx_c(x), c)
\delta(\tilx, \tilx_c(x))
\\
&=&
...
\beq
\TIL{\caly} = \sum_{x,c} \TIL{\caly}_{|x, c}P(x|c)P(c)
\eeq
...
&=&
\sum_{y,x,c}y
 [P(y|x,c)-P(y|\ul{\tilx}=\tilx_c(x), c)]P(x,c)
\\
&=&
\sum_{y,x,c}yP(y,x,c)\frac{\sum_j \indi(x\in \calx_c^j)}{P(\rvx\in \calx_c)}
[1-\frac{P(y|\ul{\tilx}=\tilx_c^j(x), c)}{P(y|x,c)}]
\eeqa
...
\beq
P(\s|j) = \frac{\calx_{c_\s}^j}
{\sum_\s\calx_{c_\s}^j}
\eeq

\beq
\av{\xi_\s}_\s= \sum_\s P(\s|j)\xi_\s
\eeq
...
\beq
\calx_c = \cup_{j=1}^{nj(c)}\calx_{c}^j
\eeq

\beq
\tilx_c(x) =\sum_{j=1}^{nj(c)}  \indi(x\in \calx_c^j)\tilx_c^j(x)
\eeq

\beq
P(\tilx|x, c)=\delta(\tilx, \tilx_c(x))=
\sum_j \indi(x\in \calx_c^j)
\delta(\tilx, \tilx^j_c(x))
\eeq
...
\lam_c(x)=
\sum_j \indi(x\in \calx_c^j)
\frac{P(\rvx=\tilx^j_c(x)|c)}{P(\ul{\tilx}=x|c)}(\tilx^j_c)'(x)
\eeq

\beqa
\TIL{\caly}
&=&
\sum_x \sum_c\caly_{|\tilx_c(x), c}P(x,c)
\\
&=&
E[\caly_{|\ul{\tilx}, \rvc}]
\eeqa
