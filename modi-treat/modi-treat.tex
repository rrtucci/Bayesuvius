...
For $G_{im2}$, $\tilx\in val(\ul{\tilx})=
val(\rvx)=\bool$.} (
As usual in this book, $val(\rva)$
denotes the set of
...
&pa'(\rvy)\ar[d]
\\
&&[\rvy(\tilx)]_{\tilx\in val(\ul{\tilx})}\ar[d]
\\
\rvx
&\ul{\tilx}=\tilx\ar[r]
&\rvy
...
$\rvy$, excluding $\rvx$.
Note that $val(\ul{\tilx})\subset val(\rvx)$. }
...
\beq \color{blue}
P(y\mid \tilx, \{y(\tilx')\}_{\tilx'\in val(\ul{\tilx})})=
\indi(
\quad y =\sum_{\tilx'\in val(\ul{\tilx})}
 y(\tilx')\indi(\tilx'=\tilx)
 \quad)
\eeq
When $val(\ul{\tilx})=\bool$,
...
to $\rvy(\tilx)$ (i.e.,
for all $\tilx\in val(\ul{\tilx})$,
we have  
$\ul{\tilx}
\perp \rvy(\tilx)|pa'(\rvy)$.)

\item {\bf Identifiability (do-Invisibility?}

We must have
\beq
val(\ul{\tilx}) \subset val(\rvx)
\eeq
in $G_{im2, mod}$
or else queries of the type
 $P(\rvy(\tilx)=y|pa'(\rvy)=\xi)$ are undentifiable
 (do-visible?).
...
for $x\in val(\rvx)$ and $\tilx \in val(\ul{\tilx})$:
...
\beq
0<g_{\tilx|c}<1 \text{ for all $\tilx \in val(\ul{\tilx})$ and $c\in val(\rvc)$}
\eeq
If $g_{\tilx|c}$
...
for some $x\in val(\rvx)$, but is not anomalous
for all $\tilx\in val(\ul{\tilx})$, where
$val(\ul{\tilx})$
is some proper subset of $val(\rvx)$.
...
Suppose $val(\ul{\tilx})\subset val(\rvx)$
and we are given a function
$\tilx_c(\cdot):val(\rvx)\rarrow val(\ul{\tilx})$.
...
\begin{itemize}
\item
Suppose we are given a function $x_c:val(\ul{\tilx})\rarrow val(\rvx)$.
...
\item
Suppose
$val(\rvx)=val(\ul{\tilx})=\bool$, $\theta\in\RR$,
and let
...
\caly_{|x, c}
&=&
\sum_y y P(\rvy(x)=y\mid x, c)
\\
&=& \sum_y y P(\rvy=y\mid x, c)
\eeqa

\beqa
\TIL{\caly}_{|\ul{\tilx}=\tilx, c}
&=&
\sum_y y P(\rvy(\tilx)=y\mid \tilx, c)
\\
&=&
\sum_y y P(\rvy=y\mid \tilx, c)
\eeqa
...
\beqa
\TIL{\caly}_{|\rvx=x, c} &=&
\sum_\tilx\sum_y y P(\rvy=y\mid \ul{\tilx}=\tilx, c)
\delta(\tilx, \tilx_c(x))
\\
&=&
\sum_y y P(\rvy=y\mid \ul{\tilx}=\tilx_c(x), c)
\\
&=&
\sum_y y P(\rvy=y\mid \rvx=x_c(\tilx), c)
\\
&=&
\caly_{|x_c(\tilx), c}
\eeqa
...
Suppose $\tilx_0, \tilx_1\in val(\ul{\tilx})$.
Define the
