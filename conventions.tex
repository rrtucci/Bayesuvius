...
When fitting
the data with the Student's t-distribution $P_S(t;
\nu=n-1)$,
the $z^*$
value is replaced 
by $t^*_{n-1}$,
...
\beq
C=\int_{-t^*_{n-1}}^{t^*_{n-1}} dz\;P_S(z) = 
\Phi_S(t^*_{n-1})-\Phi_S(-t^*_{n-1})
=
2\left(\Phi_S(t^*_{n-1})-\frac{1}{2}\right)
\label{eq-conf-level1-stu}
\;,
\eeq
where quantities subscripted by $S$
are for the Student's t distribuion,
Also, Eq.(\ref{eq-err-bars})
is replaced  by

\beq
\text{ Student's estimate of $x$
with error bars} =
\ol{x} \pm 
\underbrace{
t^*_{n-1} \frac{\hat{\s}}{\sqrt{n}}}_{E}
\;.
\eeq
Tables of $t^*_{n-1}(C)$
are available.
