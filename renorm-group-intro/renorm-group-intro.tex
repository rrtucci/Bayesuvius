...
on the topic of RG theory are listed in Section \ref{sec-rg-examples}.
...
is possible are said to be {\bf $b$-scaling or self-similar (in a continuous sense)}.
Note that
...
{d\ln b}= y_F
\eeq
so $b$ scaling functions do indeed 
possess a continuous fractal dimension\footnote{See Chapter \ref{ch-fractal-dim} for the definition of discrete fractal dimension and self similarity.} and are self-similar in a continuous rather that discrete sense.
...
$\beta(g^*=0)>0$.\footnote{In QED, $g> 0$ is the electric charge $e$
...
$\beta(g^*=0)<0$.\footnote{In QCD, $g> 0$ is the color charge
...
This once again can lead to a change in the sign of the beta function.
...
the c.c. $\beta J$ is temperature-like and r-unstable, repulsive.
...
we call that interaction term a r-unstable
...
$J$ is a r-unstable operator, and the magnetic field
