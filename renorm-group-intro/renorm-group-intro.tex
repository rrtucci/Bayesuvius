...
Another convention of ours 
is given by Eq.(\ref{eq-mu-Lam-b}).
Hence

\beq 
d\ln b = d\ln \mu = -\;d \ln \Lam
\label{eq-der-mu-Lam-b}
\eeq
 so 
\beq
\frac{d g}{d\ln \mu}= - \frac{dg}{d\ln \Lam} = \beta
\eeq
Note that some people switch
$\mu$ and $\Lam$ in Eq.(\ref{eq-der-mu-Lam-b}).
This once again can lead to discrepancies in the sign of the beta function.
...
\beq
\beta(\lam)= a(\lam-\lam^*_1)^{\nu_1}(\lam-\lam^*_2)^{\nu_2}\ldots(\lam-\lam^*_n)^{\nu_n}
\eeq
If $\lam$ and $\lam_0$ are both 
on the same side
of $\lam_1^*$
and 
\beq
...
\underbrace{a(\lam^*_1-\lam^*_2)^{\nu_2}\ldots(\lam^*_1-\lam^*_n)^{\nu_n}}_R
\eeq
Hence 
...
\\
\ln\left(
\frac{\lam-\lam_1^*}{\lam_0-\lam_1^*}\rigt)&\text{if }\nu_1=1
\end{array}
\right.
\eeq
...
model (see Chapter \ref{ch-renorm-group-phi-four}), $u = \lam$ is an i-stable c.c.. 
