...
\ecen
$$
\caption{Cubic Lattice for 1-dim (with 4 spins) and 2-dim (with 9 spins) Ising model.}
\label{fig-ising-1-2}
\end{figure}
...
{\bf n-dim Ising Model}

For $i=1, 2,3, \ldots, n$
\beq
\calh_i = J S_{i}
\sum_{j\in NN(i)}S_j + h S_{i},
\quad
\calh=\sum_{i=1}^n\calh_i
\eeq
...
\begin{itemize}
\item 1-dim Ising Model

It has no critical point. It's 
coherence length is always finite.
...
* A domain wall (spin flip boundary) costs **finite energy** (2J).
* At any (T>0), entropy favors creating arbitrarily many domain walls.
* Long-range order is destroyed immediately.


For zero external field (h=0),
\beq
\langle m \rangle = 0 \quad \text{for all } T>0
\eeq
...
\xi \sim e^{2J/kT}
\eeq
is finite for all (T>0), diverging only as $T \to 0$.

 This can be shown exactly via the **transfer matrix**.

\item 2-dim Ising model

Critical temperature (square lattice, $h=0$):
  \beq
  kT_c = \frac{2J}{\ln(1+\sqrt{2})}
  \eeq

$T < T_c$: spontaneous magnetization
...
\section{Scaling the Bnet for an Ising Model}
...
\\
\calh_2 = J S_2^Y(S_1^X + S_3^X) + h S_2^Y -\lam_2(S_2^Y-S_2^X),\quad
\lam_2\rarrow \infty
\\
...
\\
\calh_i = J S_i^Y(S_2^X) + h S_i^Y -\lam_i(S_i^Y-S_i^X),\quad
\lam_i\rarrow \infty
\\
...
\\
\calh_i = J S_i^Y(S_i^X+S_a^X+ S_b^X, S_c^X+ S_d^X) + h S_i^Y -\lam_i(S_i^Y-S_i^X),\quad
\lam_i\rarrow \infty
\\
Z_i = \sum_{S^Y_i\in \ZZ_{\pm}}e^{-\beta\calh_i}
\end{array}
...
Instead of taking the $\lam_i\rarrow \infty$,
\beq
\left\{
\begin{array}{l}
\color{blue}
P(S_i^Y\mid S_i^X, S_a^X, S_b^X, S_c^X, S_d^X)
=\frac{e^{-\beta\calh_i}\delta(S^Y_i, S^X_i)}{Z_i}
\\
\calh_i = J S_i^Y(S_i^X+S_a^X+ S_b^X, S_c^X+ S_d^X) + h S_i^Y
\\
Z_i = \sum_{S^Y_i\in \ZZ_{\pm}}e^{-\beta\calh_i}\delta(S^Y_i, S^X_i)
\end{array}
\right.
\eeq
