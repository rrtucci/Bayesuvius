...
\\
\calh_2 = -J S_2^Y(S_1^X + S_3^X) - h S_2^Y -\lam(S_2^Y-S_2^X)^2
\\
...
\\
\calh_i = -J S_i^Y(S_2^X) - h S_i^Y -\lam(S_i^Y-S_i^X)^2
\\
...
\\
\calh_i = -J S_i^Y(S_i^X+S_a^X+ S_b^X, S_c^X+ S_d^X) - h S_i^Y -\lam_i(S_i^Y-S_i^X)^2
\\
Z_i = \sum_{S^Y_i\in \ZZ_{\pm}}e^{-\beta\calh_i}
\end{array}
\right.
