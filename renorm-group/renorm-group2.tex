...
$\rvy$
is a deterministic
function of $\rvx$ (i.e., $\rvy=f(\rvx)$). In that case,
$\ln P(y|x)$ is either $0\ln(0)$
or $\ln(1)$ so 
...
\item $\rvx$ 
is a deterministic function of $\rvy$ 
(i.e., $\rvx = g(\rvy)$).
In that case,
$\ln P(x|y)$ is either $0\ln(0)$
or $\ln(1)$ so
...
Averaging the following chain rule for conditional
probabilities

\beq
\frac{P(x, a_1, a_2, a_3)}{P(x)P(a_1, a_2, a_3)}=
\frac{P(x, a_1\mid a_2, a_3)}{P(x\mid a_2, a_3)P(a_1\mid a_2, a_3)}
\frac{P(x, a_2\mid a_3)}{P(x\mid a_3)P(a_2\mid a_3)}
