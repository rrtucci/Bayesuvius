...
Choose any node as root node, and point arrows away 
from it.\\ 
Repeat this until all edges have directions:\\
\hspace*{1cm}Point arrows away from children. 
\end{enumerate}
...
\beq
\text{nn}(\rva)=\text{ number of neighbors of $\rvx$}
\eeq
...
With each of those trees,
create a TAN bnet.
For each of those TAN bnets,
calculate its
score given by
  Eq.(\ref{eq-score-tan}).
Keep
