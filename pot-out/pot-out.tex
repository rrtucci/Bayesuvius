...
are observed, but the right side
of Eq.(\ref{eq-po-ate-change})
...
balanced stratum and the map $m(\cdot)$
...
If a quantity (estimand)
 can be estimated,
it is said to be {\bf do-weak}
 (i.e., expressible without do$(\cdot)$ operators).
If positivity is violated,
 then
 $ACE=ATE$ is not do-weak.
