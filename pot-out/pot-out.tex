...
Note that in our
notation,
we ever use $Y_i(d)=\rvy^\s(d)$. 
We use $\rvy^\s(\td)$ instead..
}
...
\beq Y_i^{obs}=D_i Y_i(1) + (1
-D_i)Y_i(0)\;.
\eeq
In our notation,
this is saying that

\beq (\rvy^\s)^{obs}= \rvtd^\s \rvy^\s(\td^\s=1) +
 (1-\rvtd^\s)\rvy^\s(\td^\s=0)
\;.
\eeq
...
A treatment dose $\rvd^\s$ is said to be {\bf
ignorable} if
...
When treatment $\td$ is ignorable, we have
...
A treatment dose $ \rvd^\s$ is
 said to be {\bf conditionally
ignorable} 
 if
...
\widehat{ATT}
&=&
\overbrace{\frac{1}{N_1}\sum_\s d^\s y^\s}^{\caly_{1|1}}
 - 
\overbrace{\frac{1}{N_1}\sum_\s d^\s y^{s(\s)}}^{\caly_{0|1}}
\\
&=&
...
\widehat{ATU}
&=&
\overbrace{\frac{1}{N_0}\sum_\s (1-d^\s) y^{s(\s)} }^{\caly_{1|0}}
 - 
\overbrace{\frac{1}{N_0}\sum_\s (1-d^\s)y^\s}^{\caly_{0|0}}
\\
&=&
