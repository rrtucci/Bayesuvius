...
\beqa
\underbrace{\caly_{|1}-\caly_{|0}}_{ACE}&=&
\caly_{1|1} + \caly_{0|1}
-(\caly_{1|0} + \caly_{0|0})
\\
&=&\underbrace{\caly_{1|1}-\caly_{0|0}}_{SDO}
-\underbrace{(\caly_{1|0}-\caly_{0|0})}_{ATU}
+\underbrace{\caly_{0|1}-\caly_{0|0}}_{SB}
\label{eq-ace-decomp}
\eeqa
...
\cellcolor[HTML]{ECF4FF}$\td^\s=0$ & \begin{tabular}[c]{@{}l@{}}$E_{|x}[(1-\rvtd^\s) \rvy^\s]=E[1-\rvtd^\s]\caly_{0|0,x}$\\ $E_{|x}\left[\frac{1}{N_{0,x}}\sum_\s (1-\rvtd^\s)\rvy^{\s}\right]=\caly_{0|0,x}$\end{tabular} & \begin{tabular}[c]{@{}l@{}}$E_{|x}[(1-\rvtd^\s )\rvy^{s(\s)}]=E[1-\rvtd^\s]\caly_{1|0,x}$\\ $E_{|x}\left[\frac{1}{N_{0,x}}\sum_\s (1-\rvtd^\s) \rvy^{s(\s)}\right]=\caly_{1|0,x}$\end{tabular} \\ \hline
\cellcolor[HTML]{ECF4FF}$\td^\s=1$ & \begin{tabular}[c]{@{}l@{}}$E_{|x}[\rvtd^\s \rvy^\s]=E[\rvtd^{s(\s)}]\caly_{0|1,x}$\\ $E_{|x}\left[\frac{1}{N_{1,x}}\sum_\s \rvtd^\s \rvy^{s(\s)}\right]=\caly_{0|1,x}$\end{tabular} & \begin{tabular}[c]{@{}l@{}}$E_{|x}[\rvtd^\s \rvy^\s]=E[\rvtd^\s]\caly_{1|1,x}$\\ $E_{|x}\left[\frac{1}{N_{1,x}}\sum_\s \rvtd^\s \rvy^\s\right]=\caly_{1|1,x}$\end{tabular} \\ \hline
\end{tabular}
\caption{Expected Values
of the type $E_{|x}[\rvtd^\s \rvy^\s]$.}
...
\beq
\widehat{SDO}=
\overbrace{\frac{1}{N_{1,x}}\sum_\s \td^\s y^\s}^
{\caly_{1|1,x}}
-
\overbrace{\frac{1}{N_{0,x}}\sum_\s (1-\td^\s) y^\s}^
{\caly_{0|0,x}}
\eeq
...
\widehat{ATT}
&=&
\overbrace{\frac{1}{N_{1,x}}\sum_\s \td^\s y^\s}^{\caly_{1|1,x}}
 - 
\overbrace{\frac{1}{N_{1,x}}\sum_\s \td^\s y^{s(\s)}}^{\caly_{0|1,x}}
\\
&=&
\frac{1}{N_{1,x}}\sum_\s \td^\s [y^\s - y^{s(\s)}]
\label{eq-est-att}
\eeqa
...
\widehat{ATU}
&=&
\overbrace{\frac{1}{N_{0,x}}\sum_\s (1-\td^\s) y^{s(\s)} }^{\caly_{1|0,x}}
 - 
\overbrace{\frac{1}{N_{0,x}}\sum_\s (1-\td^\s)y^\s}^{\caly_{0|0,x}}
\\
&=&
\frac{1}{N_{0,x}}\sum_\s (1-\td^\s) [ y^{s(\s)}-y^\s]
\label{eq-est-atu}
\eeqa
...
\\
&=&
\frac{1}{N_x}\sum_\s (2\td^\s-1)[y^\s -y^{s(\s)}]
\label{eq-est-ate}
\eeqa
...
\frac{\td^\s}{N_{1,x}}
\rarrow
\frac{\td^\s}{N_x g_1(x^\s)}
\;,\;\;
\frac{1-\td^\s}{N_{0,x}}
\rarrow
\frac{1-\td^\s}{N_x g_0(x^\s)}
\;.
\eeq
These replacements are
equal under $ E_{|x}\sum_\s$
to the terms they replace:
\beq
E_{|x}\left[\sum_\s\frac{\rvtd^\s}{N_{1,x}}\right]
=
E_{|x}\left[\sum_\s\frac{\rvtd^\s}{N_x g_1(\rvx^\s)}\right]=1
\;,\;\;
E_{|x}\left[\sum_\s\frac{1-\rvtd^\s}{N_{0,x}}\right]
=
E_{|x}\left[\sum_\s\frac{1-\rvtd^\s}{N_x g_0(\rvx^\s)}\right]=1
\;.
\eeq
