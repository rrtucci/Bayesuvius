...
\rarrow
E_{\rvy|d,x} [\rvy(\td)]
=\sum_{y} yP_{\rvy(\td)|\rvtd=d,x}(y|d,x)
...
E[Y(d)|x]$$.}\label{cl-caly-bardx}
\beq
\caly_{|d, x}=\caly_{d|d,x}=\caly_{d|x}
\eeq
\end{claim}
\proof
...
\\
&=&
\sum_x [\caly_{|1,x}-\caly_{|0,x}]P(x)\;\;
\text{(by the backdoor criterion.)} 
\eeqa
...
By Claim \ref{cl-caly-bardx},
 $\caly_{\td|d, x}=\caly_{\td|x}$ so

\beqa
ACE&=&\sum_x[\caly_{1|x}-\caly_{0|x}]P(x)
\\
&=&
\caly_1-\caly_0=ATE
\eeqa
...
\beq 
\rvy^\s(\rvtd)= \rvtd \rvy^\s(1) +
 (1-\rvtd)\rvy^\s(0)
\;.
\eeq


\section{${\cal Y}_{\tilde{d}|d}$
...
\eeq
Hence,
if we replace every $\caly_{\td|d}$
by $\caly_{\td|d,x}$ in
Fig.\ref{fig-y-diffs-square}, the square
in that figure
collapses to 

\begin{subequations}\label{eq-effects-cia}
\beq
\xymatrix{
\caly_{0|x}\ar[r]_{SDO_x}&
 \caly_{1|x}
}
\eeq 
Hence,
...
\beq
SDO_x=ATT_x=ATU_x=ATE_x
\;.
\eeq
\end{subequations}

If we assume a RCT, then Eqs.(\ref{eq-effects-cia})
are all valid without the $x$ conditioning. Hence,
...
Despite the fact that Eqs.(\ref{eq-effects-cia})
are true, they are only
valid for very large numbers $N_x$
of individuals $\s$
with $\rvx^\s=x$. 
Later on we will 
give estimators $\hat{\cale}_x$
for the effects $\cale_x$. We will
not assume that 
Eqs.(\ref{eq-effects-cia}) are valid 
for the estimators $\hat{\cale}_x$ too.
For example, we won't assume that
$\widehat{SB}_x=0$ and 
$\widehat{SDO}_x=
\widehat{ATE}_x$.
...
$\xymatrix{\caly_0\ar[r]_{SDO}&\caly_1}$.
Therefore, for a RCT, $SDO=ATE$.
Hence, $SDO=ATE=0$
...
distribution $P$.
\beq
\caly_{\td|d}=\sum_{y} y P_{\rvy(\td)|\rvtd}(y|d)
\eeq
for $d, \td\in \bool$.
Fig.\ref{fig-y-diffs-square-probs}
...
Recall also that $SDO=ATE$ for a RCT.
$SDO=0$ is the null 
hypothesis in a RCT. 
...
such that,
for each $x$,
$a(A_{0,x})\subset A_{1,x}$ and
$a(A_{1,x})\subset A_{0,x}$.
This assumes $A_{0,x}$ and $A_{1,x}$
are non-empty for all $x$.
The purpose of map $a()$
is
...
\cellcolor[HTML]{ECF4FF}$\td^\s=0$ & $y^\s$ & $y^{a(\s)}$ \\ \hline
\cellcolor[HTML]{ECF4FF}$\td^\s=1$ & $y^{a(\s)}$ & $y^\s$ \\ \hline
\end{tabular}
\caption{Illustration of the
purpose of the map $a()$.
Note that $y^\s=y^\s(\td^\s)$ and $y^{a(\s)}=y^\s(!\td^\s)$,
where $!0=1$ and $!1=0$.}
\label{tab-po-s-map}
\end{table}
...
\hline
 & \cellcolor[HTML]{ECF4FF}$y^\s(0)$ & \cellcolor[HTML]{ECF4FF}$y^\s(1)$ \\ \hline
\cellcolor[HTML]{ECF4FF}$\td^\s=0$ & \begin{tabular}[c]{@{}l@{}}$E_x[(1-\rvtd^\s) \rvy^\s]=E[1-\rvtd^\s]\caly_{0|0,x}$\\ $E_x\left[\frac{1}{N_{0,x}}\sum_{\s\in A_x} (1-\rvtd^\s)\rvy^{\s}\right]=\caly_{0|0,x}$\end{tabular} & \begin{tabular}[c]{@{}l@{}}$E_x[(1-\rvtd^\s )\rvy^{a(\s)}]=E[1-\rvtd^\s]\caly_{1|0,x}$\\ $E_x\left[\frac{1}{N_{0,x}}\sum_{\s\in A_x} (1-\rvtd^\s) \rvy^{a(\s)}\right]=\caly_{1|0,x}$\end{tabular} \\ \hline
\cellcolor[HTML]{ECF4FF}$\td^\s=1$ & \begin{tabular}[c]{@{}l@{}}$E_x[\rvtd^\s \rvy^\s]=E[\rvtd^{a(\s)}]\caly_{0|1,x}$\\ $E_x\left[\frac{1}{N_{1,x}}\sum_{\s\in A_x} \rvtd^\s \rvy^{a(\s)}\right]=\caly_{0|1,x}$\end{tabular} & \begin{tabular}[c]{@{}l@{}}$E_x[\rvtd^\s \rvy^\s]=E[\rvtd^\s]\caly_{1|1,x}$\\ $E_x\left[\frac{1}{N_{1,x}}\sum_{\s\in A_x} \rvtd^\s \rvy^\s\right]=\caly_{1|1,x}$\end{tabular} \\ \hline
\end{tabular}
...
&=&
\frac{1}{N_x}
\left[\sum_{\s\in A_x} \td^\s [y^\s - y^{a(\s)}]+
\sum_{\s\in A_x}(1-\td^\s) [ y^{a(\s)}-y^\s]
\right]
\\
&=&
\frac{1}{N_x}\sum_{\s\in A_x} (2\td^\s-1)[y^\s -y^{a(\s)}]
\label{eq-est-ate}
\eeqa
...
\overbrace{\frac{1}{N_{1,x}}\sum_{\s\in A_x} \td^\s y^\s}^{\caly_{1|1,x}}
 - 
\overbrace{\frac{1}{N_{1,x}}\sum_{\s\in A_x} \td^\s y^{a(\s)}}^{\caly_{0|1,x}}
\\
&=&
\frac{1}{N_{1,x}}\sum_{\s\in A_x} \td^\s [y^\s - y^{a(\s)}]
\label{eq-est-att}
\eeqa
...
\widehat{ATU}_x
&=&
\overbrace{\frac{1}{N_{0,x}}\sum_{\s\in A_x} (1-\td^\s) y^{a(\s)} }^{\caly_{1|0,x}}
 - 
\overbrace{\frac{1}{N_{0,x}}\sum_{\s\in A_x} (1-\td^\s)y^\s}^{\caly_{0|0,x}}
\\
&=&
\frac{1}{N_{0,x}}\sum_{\s\in A_x} (1-\td^\s) [ y^{a(\s)}-y^\s]
\label{eq-est-atu}
\eeqa
...
$a()$. Only the estimator for $SDO_x$
is valid because it doesn't depend on $a()$.
...
\hline
\cellcolor[HTML]{ECF4FF} $\s$& \cellcolor[HTML]{ECF4FF}$\td^\s$ & \cellcolor[HTML]{ECF4FF}$y^\s$ & \cellcolor[HTML]{ECF4FF}$\td^\s y^\s$ & \cellcolor[HTML]{ECF4FF}$(1-\td^\s)y^\s$ & \cellcolor[HTML]{ECF4FF}$\td^\s y^{a(\s)}$ & \cellcolor[HTML]{ECF4FF}$(1-\td^\s)y^{a(\s)}$ \\ \hline
...
contains exactly 5 elements $a$.
Thus, $\calm_{\eps(\s, 5)}(\s)$
contains the $a$ of the
 5 points $x^a$ that are the
...
\;,
\eeq
where $\Sigma = \av{\rvx^\s, [\rvx^a]^T}$.
 This
metric $dist(x^\s, x^a)$ is
...
that the estimator
Eq.(\ref{eq-est-att})
for $ATT_x$ divides by $N_{1,x}=P(d=1|x)N_x$
and 
the estimator
Eq.(\ref{eq-est-atu})
for $ATU_x$ divides by $N_{0,x}= P(d=0|x)N_x$.
