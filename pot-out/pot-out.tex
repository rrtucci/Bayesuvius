...
for
$X\in \{d, x, y\}$ 
and $\s=\{0,1,2, \ldots, nsam-1\}$.


For each unit (aka individual, sample) 
...
$\caly_{0|0}, \caly_{1|1}$
are {\bf factual} (i.e., complaint)
whereas 
$\caly_{0|1}, \caly_{1|0}$
are {\bf counterfactual} (i.e., non-complaint).
...
PO doesn't use $\caly_{|\td}$ so often
$E[Y_i(d)]$ and 
$E[Y_i|D_i=d]$ are both used to mean $\caly_d$.
I find
...
\;.
\eeq
This is satisfied by $G_{do}$, as implied by 
the d-separation theorem 
...
\;.
\eeq
This is satisfied by $G_{im}$, as implied by 
the d-separation theorem 
(see Chapter \ref{ch-dsep})
after checking that
...
$\xi(\vec{d})=1$
for the true (i.e., $H_0$
satisfying)  vector $\vec{d}=\vec{d}_0$,
and $\xi(\vec{d})=2$ for
one possible
...
We end this section
by calculating $P(y,d)$
for the bnet $G_{ps}$.

From the TPMs, one has

\beqa
\sum_{g^\s}P(d^\s|g^\s)P(g^\s|x^\s)
&=&
g(x^\s) d^\s + (1-g(x^\s))(1-d^\s)
\\
&=&
P(d^\s|x^\s)
\;.
...
\\
&=&
E_x E_{g|x}P(y|d,x)P(d|g)
\\
&=&
E_g E_{x|g}\left[P(y|d,x)\right]P(d|g)
\\
&=&
E_g P(y|d,g)P(d|g)
\label{eq-ps-g-strata}
\eeqa
where
...
\beqa
P(y|d,g)&=& E_{x|g}P(y|d,x)
\\
&=&
\frac{1}{P(g)}
\sum_x \delta(g, g(x))P(x)P(y|d,x)
\;.
\eeqa

Using Eq.(\ref{eq-ps-g-strata}),
one is equipped to do
