...
\beq\color{blue}
P(y^\s(0)|\td^\s,d^\s,x^\s) = 
P(\rvy=y^\s(0)|\rvtd^\s=0,d^\s, x^\s)
\eeq

\beq\color{blue}
P(y^\s(1)|\td^\s,d^\s,x^\s) = 
P(\rvy=y^\s(1)|\rvtd^\s=1,d^\s, x^\s)
\eeq
...
\begin{array}{l}
\indi(y^\s= y^\s(\td^\s=1))\\
P(y^\s(0)|\td^\s=1,d^\s,  x^\s)\\
P(y^\s(1)|\td^\s=1, d^\s, x^\s)
\end{array}
\right.
\\
&=
...
\label{eq-y-is-y-td}
\end{align}
and similarly when $\td^\s=0$.
Note that
$P(y|\td^\s, d^\s, x^\s)$
for $d^\s\in \bool$
and $\td^\s\in \bool$
constitutes 4 different 
probability distributions.
When $d^\s\neq \td^\s$,
we say it's a counterfactual
distribution (it can never be measured,
...
E_{\rvy|d,x} [\rvy]
=\sum_{y} y
\underbrace{P_{\rvy|\rvtd=d,x}(y|d,x)}_
{P_{\rvy(d)|\rvtd=d,x}(y|d,x)}
\eeq
...
E_{\rvy|x} [\rvy(\td)]
=\sum_d \caly_{\td|d,x}
\underbrace{P(\rvtd=d|x)}_
{=P(\rvtd=d) \text{ by d-separation}}
\eeq
...
& 
$\frac{1}{N_{0,x}}\sum_{\s\in A_x} (1-\rvtd^\s)\rvy^{\s}=\caly_{0|0,x}$
& 
$\frac{1}{N_{0,x}}\sum_{\s\in A_x} (1-\rvtd^\s) \rvy^{m(\s)}=\caly_{1|0,x}$
\\ \hline
\cellcolor[HTML]{ECF4FF}$\td^\s=1$ 
&
 $\frac{1}{N_{1,x}}\sum_{\s\in A_x} \rvtd^\s \rvy^{m(\s)}=\caly_{0|1,x}$
& 
$\frac{1}{N_{1,x}}\sum_{\s\in A_x} \rvtd^\s \rvy^\s=\caly_{1|1,x}$
\\ \hline
\end{tabular}
