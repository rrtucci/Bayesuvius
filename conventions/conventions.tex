...
For $p\in [0, \infty]$
and $\vec{x}\in\RR^p$ or $\vec{x}\in\CC^p$,  
...
\beq
\norm{\vec{x}}_0 = \sum_i \indi(|x_i| >0) = \text{number of non-zero $x_i$}
\eeq

\beq
\norm{\vec{x}}_1 = \sum_i |x_i|
\eeq

\beq
\norm{\vec{x}}_2 = \sqrt{\sum_i (x_i)^2}
\eeq

\beq
\norm{\vec{x}}_3 = \left(\sum_i |x_i|^3
\right)^\frac{1}{3}
\eeq
...
operations
used in Physics (Elasticity, Fluid Mechanics, General Relativity and Quantum Mechanics all use tensors. Tensors in various guises are ubiquitous in Physics.)
to the tensor operations
used in Machine Learning (ML) as exemplified by Numpy.
 
 But first, some notation.
 
For integers $M\geq N$, let
\beq
\rho(N,M) = [N, N+1, \ldots, M-2, M-1]=[N:M]
\eeq
and 
\beq
\rho(N) = \rho(0,N)=[0:N]
...
$\rho()$ acts the
the same way as 
the function {\tt range()} in Python.

We will also use
\beq
A^{\rho(N)} = (A^0, A^1, \ldots A^{N-1})
\eeq
and
...
will usually use upper case letters for space dimensions like $N$ in $i^N$,
and lower case letters for indices
like $i$ in $i^N$.

We will use $i$ and $i^N$ to represent
the same thing, an
element of $\rho(N)$,
an index value.
The second notation is more explicit but optional.

For {\bf scalars} $\alp, \beta\in\RR$, the {\bf linear combination of tensors} $T, S$ is defined by



\beq
[T^{i,j,k}, S^{i,j,k}]
\rarrow \alp T^{i,j,k} + \beta S^{i,j,k}
\eeq
...
\beq
S_j = \sum_i S^i \delta_{i,j}
\eeq

The {\bf Einstein implicit summation convention} is the
practice of omitting the summation sign, assuming  that repeated indices are summed over if 
one index is covariant (upper)
and the other is contravariant (lower).
\beq
S^iT_i = \sum_i S^i T_i
\eeq
Sums between a lower and an upper index are called {\bf contractions}.

$dim$
 refers to the positions
 of indices in a tensor (e.g., $dim=0$ for $i$, $dim=1$ for $j$ and 
 $dim=2$ for $k$ in $T^{i,j,k}$).
$axis\in \rho(N)$ 
refers to the values of an index  along a
particular dimension
(e.g., we say $i$ is along $dim=0$ in $T^{i^N, j,k}$). 
 For a 2-dim array: (1)
...
division $(+,-,*,/)$ of 2 tensors with the same shape, or a tensor and a scalar.
...
discarding tensor entries}.
\end{enumerate}
Out of these three categories, Physics uses (1) often, and
 (2), (3) rarely. ML, on the other hand, uses all three methods frequently. This is
probably due to the fact
that Physics loves linearity, and (3) is not a linear transformation, but (1) is.

Next I will discuss in a visual manner\footnote{The Numpy methods in this list are discussed here purely visually and analytically, without numerical examples. For numerical examples, see the excellent Numpy docs.}, a random
assortment of Numpy 
...
use below my own way of visualizing Numpy tensor methods. Namely, I will 
...
\item {\bf expand\_dims()} (i.e., unsqueeze) along $dim=0$
...
\beq
T^{
i^{N_0},j^{N_1},k^{N_2}}\rarrow
T^{i_1, j_1, k_1}
\delta_{a^N(i_1, j_1, k_1)}^{b^N(i,j,k)}
\eeq
where $N=N_0N_1N_2$.

\item {\bf gather()}\footnote{This operation comes from
pytorch. Numpy doesn't have it.} along $dim=0$
...
\beq
[T^{\rho(N),j,k}, S^{\rho(N),j,k}]\rarrow 
T^{i,j,k}\delta_{i,i'} S^{i',j,k}
\eeq
...
$\sigma:\rho(3)\rarrow \rho(3)$. \footnote{ a permutation $\s:\rho(N) \rarrow \rho(N)$ is a bijection, i.e., a 1-1 onto map.}

\beq
T^{i,j,k} \rarrow T^{i_1, j_1, k_1}
\delta_{i_1, j_1, k_1}^{\sigma(i, j, k)}
\eeq
For example,

\beq
T^{i,j,k} \rarrow T^{i_1, j_1, k_1}
\delta_{i_1, j_1, k_1}^{k,j,i}
\eeq
