...
\section{Estimand, Estimator, Estimate, Bias}
\label{sec0-estimand}
For an {\bf estimand} $\theta$,
an {\bf estimator} $\ul{\hat{\theta}}$
...
And sometimes the estimate $\theta + b$
is represented by a $\hat{\theta}$ too.
In some sense, an estimator is an estimate
of a curve, so it's understandable,
and not too confusing, that
the terms ``estimator" and ``estimate"
are used interchangeably,
as  long as both terms are not being used
concurrently in the same explanation.
In case both are being used
in the same explanation, just remember that an estimator
is a function that you eventually intend to average
over whereas an estimate is just
a real value. And if you
use a letter with a hat,
use it to represent either the estimator or
the estimate, but not both.
