...
Most bnets in this book were drawn using the LaTex package {\tt xy-pic}, or the Python app {\tt texnn} (Ref.\cite{texnn}). {\tt texnn} is a Python wrapper for 
...
are {\bf categorical sets} (a.k.a. \enquote{nominal sets}, i.e.,
sets with \enquote{named} elements, with elements given a \enquote{nomme})
and {\bf numerical sets} (a.k.a. \enquote{ordinal sets}, i.e., sets
 with \enquote{ordered}
elements).
...
\eeq
\enquote{$|a$} means that the variable
...
A better name for it would have been \enquote{soft Kronecker
delta function}.
...
is why \enquote{smoid} is also called \enquote{expit}.
...
which is why \enquote{lodds} is also called \enquote{logit}.
...
Unfortunately, the
words \enquote{estimator} and
\enquote{estimate} are often used interchangeably,
...
the terms \enquote{estimator} and \enquote{estimate}
...
$\HAT{\theta}$. Note that\footnote{\enquote{sup} stands for supremum.
...
Likewise, \enquote{inf} stands for infimum,
...
Next, assume
{\bf  \enquote{homoscedasticity} (homo-spread)}\footnote{I
 find the word \enquote{homoscedasticity}
unnecessarily long, cryptic
and easy to misspell so
I like to replace
it by \enquote{homo-spread}.
The opposite
of \enquote{homoscedasticity}
is \enquote{heteroscedasticity},
which I like to replace with \enquote{hetero-spread}.}, which
means that
...
$(\hatvar)_\infty$ the
\enquote{population variance}. I will
...
by $n$ instead of $n-1$.
In terms of \enquote{degrees of freedom}(DOFs),
...
is explained in the next section entitled \enquote{Confidence Intervals}.
...
believe that $h$
is a \enquote{parameter} which has an priori value; therefore,
...
Here is some 
\enquote{Heisenberg uncertainty
...
can be \enquote{sanity checked} by
replacing
$z=e^{sT}$
and checking that
for $0<T<<1$,
...
one replaces the words \enquote{concave}
by \enquote{convex} and
\enquote{min} by \enquote{max}.\footnote{This
book does not try to be
mathematically rigorous
beyond the level
of applied math. To be truly
rigorous and general,
replace \enquote{max} by \enquote{supremum}
and \enquote{min} by \enquote{infimum}.
...
words \enquote{concave} with \enquote{convex}
and \enquote{min} with \enquote{max}.
\end{claim}
\proof
...
use a graphical box (what I call a \enquote{box of puzzle pieces}) to indicate
