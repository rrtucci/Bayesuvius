...
Technically,
min and max should
only be used for finite sets,
but physicists and engineers 
often discretize over a finite set.}
$\TIL{f}(p)$
is also called the 
...
Let $x^*$ 
be the value of $x$
which minimizes $\TIL{f}(p)=p^T x-f(x)$
...
Let $p^*$ 
be the value of $p$
which minimizes $f(x)=p^T x-\TIL{f}(p)$
