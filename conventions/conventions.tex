...
\beq
\rvx_1\sim \caln(x_1; \mu_1, \s^2_1)
\eeq
and

\beq
\rvx_2\sim \caln(x_2; \mu_2, \s^2_2)
\eeq
then
\beq 
\rvx=\rvx_1 +\rvx_2 \sim \caln(x;\mu_1 + \mu_2, \s^2_1 + \s^2_2)
\;.
\eeq
\end{claim}
\proof
...
{\bf Weighted LR} 
when $W\neq 1$
and simple LR  when $W=1$.
\begin{claim}
Assume the 
Einstein summation convention; i.e., 
repeated indices must be summed over. 
The 
...
\;.
\eeq
Define the $n$ dimensional matrix $C$
by
...
If assume that the $x_\s$ are i.i.d.,
...
\;.
\eeq
In the derivative notation,
Eq.(\ref{eq-beta-i-non-deriv}) becomes:
...
STEP 2: Regress $\caly_i$ on $\rvx_i$.
