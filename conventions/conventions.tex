...
\beq
R=-2\ln 
\left(\frac{\sup_{\theta\in S_0}\call(\theta)}
...
$H_0$: $\theta=\theta^*$,
and an {\bf alternate hypothesis}
 $H_1$: $\theta\neq \theta^*$.
...
are (See Fig.\ref{fig-classic-trio}):\footnote{We
are using the 
Einstein summation
...
(\hat{\theta}-\theta^*)_i
\left[
\av{\hat{\rvtheta}_i,\hat{\rvtheta}_j}
\right]_{i,j}^{-1}
(\hat{\theta}-\theta^*)_j
\\
&=&
...
$\av{\hat{\rvtheta}_i,\hat{\rvtheta}_j}$
is close to 
$E[\partial_{\theta_i}\partial_{\theta_j}
 P(\vec{\rvx}|\hat{\theta})]$.
