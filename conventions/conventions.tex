...
Note that
\beq
P(\rvx=x)=\lim_{\Delta x\rarrow 0}
\frac{P(\rvx<(x+\Delta x))-P(\rvx<x)}{\Delta x}
=\frac{dP(\rvx<x)}{dx}
\eeq
...
&=& \int_0^1 x \;dP(\rvx<x)
\\
&=&\int_0^1 \left[d(xP(\rvx<x))- dx\; P(\rvx<x)
\right]
\\
&=&
...
The function
softmax$(\cdot)$ is called softmax because if we
...
\frac{1}{1+e^{-x}}
\eeq
$\smoid(\cdot)$ is monotonically
increasing with $\smoid(-\infty)=0$,
$\smoid(0)=1/2$
and $\smoid(+\infty)=1$.
Note that for $x\ll 0$, $\smoid(x)\approx e^x$, which
...
\eeq
Note that for $0< p\ll 1$, $\lodds(x)\approx \ln p$,
which is why \qt{lodds} is also-called \qt{logit}.

Note that for $x\ll 1$, $\smoid(x)\approx e^x\ll 1$,
...
\eeq
Hence,
$\lodds(\cdot)$ is the inverse of $\smoid(\cdot)$ and vice versa.
...
It's a generalization of the function $\max(\cdot)$
to arbitrary sets
...
and it generalizes the $\min(\cdot)$ function.}
...
\begin{claim}
If $\left|\frac{PO(x)}{PE(x)}-1\right|\ll 1$
...
$\caln(z; \mu=0, \sigma=1)$.
Stud$(\cdot)$ is symmetric about
the origin like SND,
but its tails
are fatter.
When fitting the data with Stud$(\cdot)$,
the $|z^*|$
value is replaced
by a $|t^*|$ value.
...
believe that $h$
is a \qt{parameter} which has an a priori value; therefore,
...
$\caln(x;\mu=0, \s=0)$.
For $n<30$, $\Phi(\cdot)$ is replaced
by $\Phi_S(\cdot)$, where  $\Phi_S(\cdot)$ is
...
on the right sides.\footnote{Note the
...
\item {\bf concatenate$(\cdot)$} along $dim=0$
...
\item {\bf expand\_dims$(\cdot)$} (same as {\bf unsqueeze$(\cdot)$}) along $dim=0$
...
\item {\bf gather$(\cdot)$}\footnote{This operation is available in
...
\item {\bf max$(\cdot)$} and {\bf argmax$(\cdot)$} along $dim=0$
...
\item {\bf repeat$(\cdot)$} with $r=[r_0, r_1, \ldots, r_{n-1}]$
along $dim=0$
...
note that concatenate$(\cdot)$ along $dim=0$ and repeat$(\cdot)$
 with $r=pow(1, n)$
along $dim=0$, are the same thing. So repeat$(\cdot)$ is a
souped up version of concatenate$(\cdot)$.


\item {\bf reshape$(\cdot)$} from shape
...
flatten$(\cdot)$ is clearly a special case of reshape$(\cdot)$.


\item {\bf split$(\cdot)$} along $dim=0$
...
\item {\bf stack$(\cdot)$} along $dim=0$
...
[3], \alpha, \beta, \gamma}
\eeq
Compare this to concatenate$(\cdot)$. stack$(\cdot)$
creates a new dimension
whereas concatenate$(\cdot)$ doesn't. concatenate$(\cdot)$ just increases the range
of an existing dimension.

\item {\bf sum$(\cdot)$} along $dim=0$

\beq
T^{[a], [b]}\rarrow \sum_{\alp} T^{\alp, [b]}
\eeq

\item {\bf tensordot$(\cdot)$} (i.e., contraction) along $dim=0$
...
\item {\bf  tile$(\cdot)$}\footnote{
tile$(\cdot)$ in Numpy corresponds
to repeat$(\cdot)$ in PyTorch.}
...
\item {\bf transpose$(\cdot)$} by a permutation
