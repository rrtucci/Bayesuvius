...
\beq\color{blue}
P(h^{\lam}_i\mid h^{\lam-1}_.)=
\delta\left(h^{\lam}_i,
\phi_i^\lam(\sum_j W^{\lam}_{i|j}
...
$W^\lam$ as follows. For each $\lam$, replace the matrix
 $W^\lam$
by
...
\beq
\phi(x)=\underbrace{x\indi(x>0)}_{x_+}= \max(0, x)
\;.
\eeq
...
\beq\color{blue}
P(y.[\sigma]\mid x.[\sigma])
\text{ = given}
\;.
\eeq

\beq\color{blue}
P(h^{\lam}_i[\sigma]\mid h^{\lam-1}_.[\sigma])=
...
\beq\color{blue}
P(Y_i[\sigma]\mid h^{\Lambda-2}_.[\sigma])=
...
\beq\color{blue}
P(h^\lam_i[\sigma]\mid h^{\lam-1}_.[\sigma])=
...
\beq \color{blue}
P(h^\lam_i[\sigma]\mid h^{\lam-1}_.[\sigma], r^\lam_i)=
\delta(h^\lam_i[\sigma],H^\lam_i[\sigma])
\;,
\eeq
where
