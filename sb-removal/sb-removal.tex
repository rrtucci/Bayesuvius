...
\section{Pre and Post Switch Nodes}

This book uses 2 types of switch nodes:
pre-switch nodes and post-switch nodes.

{\bf Pre-switch nodes} are used
in Chapter \ref{ch-transport}
entitled \enquote{Transportability
of Causal Knowledge}.
Pre-switch nodes
are {\color{red}root} nodes. They
are usually binary, and indicate
...
define SB using a {\bf post-switch node}.
A post-switch node
is a {\color {red}leaf} node
of the graph. Like pre-switch nodes,
post-switch nodes are usually
binary.
...
It's possible to replace a pre-switch node
by a post switch node, or vice versa, as
follows.
Suppose that we start with
a bnet $G_0$ that is fully connected, and
we add to it a switch node $\rvs$
...
a post-switch node $\rvs$, is converted
to a graph which is numerically
equal to it, namely
the bottom
graph
 $G_{pre-sel}$, which has
a pre-switch node $\rvs$.
...
\caption{Switching
from a post-switch node
to a pre-switch node.}
...
\enquote{observationally equivalent}.
