...
more precise
but less catchy title for this RG introduction
would have been \qt{Honey,
...
\section{Example: 1-dim Ising Model}
...
{renorm-group1/kprime-versus-k-1dim-ising.png}
\includegraphics[width=3.5in]
{renorm-group1/kprime-versus-k-2dim-ising.png}
...
\section{Example: $\phi^4$ theory}

\section{Naive versus fractal
\qt{scaling} dimensions}
...
$b$ and $\calp$ languages.
Since $1\leq b < \infty$,
it follows that
$0<\calp\leq 1$. As
$b$ goes from 1 to $\infty$,
$\calp$ goes from 1 to 0.
$b$ and $\calp$  are both 1 at the same time.
They both start at 1, but one goes up
and the other down.
The point $b=\calp=1$
corresponds to a microscopic,
high energy (ultraviolet) picture, whereas
the point $b=\infty$, $\calp=0$
corresponds to a macroscopic,
low energy (infrared)
 picture.\footnote{Hence, it is convenient to think of $b$ as akin to $\beta$
in thermodynamics. I think
that is the reason that this
compression factor
is commonly called $b$ in the literature.}
Note also that
$\ln \calp = -\ln b$
...
a length $x$ and
an energy $E$,
one has (see Fig.\ref{fig-rg-scaling})

\beq
\delta x' = \frac{\delta x}{b}=\calp \delta x
\;,
\eeq

\beq
\delta E'=b \delta E=\frac{\delta E}{\calp}
\;.
\eeq
...
{renorm-group1/rg-scaling.png}
\caption{RG scaling of length interval $\delta x$
and energy interval $\delta E$ for $b>1$.}
\label{fig-rg-scaling}
\end{figure}
...
pieces $\ln(q'/q)$ divided by log of magnification $\ln(b)$ . See
Ref.\cite{wiki-fractal}}
...
can (resp., cannot) set $D_q=-y_q$
for all $q$ are associated with lossless (resp., lossy)
...
system goes, at zero
externally applied magnetic field, from
zero to growing, spontaneously).
\item
...
The set of all points that flow into
 the critical point (i.e., the \qt{basin
...
{renorm-group1/renorm-group-critical-point.png}
...
in the direction of
increasing $b$ (i.e., decreasing $\calp$).
...
given $g_j$ and
$\ln (\calp'/\calp)$, one can find the value of
$g'_j$.
One describes this felicitous property
by saying that the c.c. $g_j$ is {\bf self-similar}
or {\bf  a running c.c.}.
...
functions $\beta_j(\calp)$
by

\beq
\frac{dK_j}{d\ln \calp} = \beta_j(\calp)
\;.
\label{eq-dif-kj}
\eeq
Then a fixed point
...
for all $j$.
If $\vec{K}^*$ occurs
when $\calp=0$ (resp., $\calp=1$),
we call it an infrared (resp., ultraviolet)
...
like $\calp$ or $b$. Instead, they
use $\ln \mu$ (or $\ln \Lambda$)
en lieu of $\ln \calp$
in Eq.(\ref{eq-dif-kj}).
That's no problem. Just replace
$\mu$ by $\calp \mu$ (or
$\Lambda$ by $\calp \Lambda$).
If you keep $\mu$
(or $\Lambda$) fixed and
allow $\calp$ to vary, then
$d\ln(\calp\mu)$ (or $d\ln(\calp\Lambda)$ )
can be replaced by $d\ln \calp$.
...
at ultra-high energy scales $\calp= b=1$.
The dressed c.c.'s
 correspond to $\vec{K}^{(n)}$
for a large $n$
and not-so-ultra-high energy scales,
$\calp\in (0,1)$ but $\calp<<1$.
The bare c.c.'s are
