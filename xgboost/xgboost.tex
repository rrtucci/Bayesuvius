...
Let us abbreviate  $s(\cdot)=\smoid(\cdot)$
and $l(\cdot)=\lodds(\cdot)$ so
...
we approximate the divergence $D(\cdot)$
...
From the function $\ell_t(\cdot)$, we can derive
...
\beq\color{blue}
P([\haty^\s_t]_\s\mid f_t , \vec{x})=
\prod_{\s\in\Sigma}
\indi(\;\;\;\haty^\s_t= f_t(x^\s)\;\;\;)
\eeq

\beq\color{blue}
P([A^\ell_t]_{\ell\in\call_t}\mid
...
\beq\color{blue}
P(f_t\mid [A^\ell_t]_{\ell\in\call_t},
