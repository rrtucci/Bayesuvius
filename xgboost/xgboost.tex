...
$\s\in \Sigma$ be individual in a population $\Sigma$
...
&=&
D(\haty^\s_{t-1}+
\underbrace{f_t(x^\s)}_\delta, y^\s)
\\
&\approx&
D(\haty^\s_{t-1}, y^\s)
+ a^\s_t \delta
...
+
\left[a^\s_t w_t^\ell
+ \frac{1}{2}b^\s_t (w_t^\ell)^2
\right]\indi(\s\in \Sigma_t^\ell)
\;,
\eeqa
where
...
\beq
b^\s_t=
[\partial^2_{\haty}D(\haty, y^\s)]_{\haty=\haty^\s_{t-1}}
\;.
\eeq
...
to a single node
or even rule out the whole
tree.

$\lam$ measures {\bf level of insensitivity}
to observations, $\gamma$ measures
...
Use the right edge $X_j$ 
of each bin in the  
 node question $x^\s<X_j?$,
...
If we expand 
the same cost function $\calc_t$
defined previously
(i.e., Eq.\ref{eq-xgb-cost-fun}))
in powers of $F_t(x^\s)=w^{\ell_t(\s)}_t$
instead of 
powers of$f_t(x^\s)$
...
[r^\s_t]_\s)=
\begin{array}{l}
\text{build tree $\ell_t$ using residuals
$r^\s_{t}=y^\s-\haty^\s_{t-1}$.}
\\
\text{$\gamma$ is used here.}
\end{array}
\eeq
