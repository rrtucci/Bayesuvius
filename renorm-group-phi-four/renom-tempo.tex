...
\beq
\lam_0 =
\mu^\eps \tilde{Z}_\lam\tilde{\lam}
\eeq
where

\beq
\tilde{Z}_\lam = 1 + \lam a\left(
\frac{1}{\eps}-b\right) +\calo(\lam^2)
\eeq

\beq
\tilde{\lam}=
\lam[1 + c\lam + \calo(\lam^2)]
\eeq
with
...
\ecen}_{\Gamma^{(4)}_{\text{1-loop}}}
+\calo(\lam^3)
\eeqa

\beq
\Gamma^{(4)}_{\text{1-loop}}=
\frac{3\lam^2}{2}
\underbrace{
\int
\frac{d^d k}{(2\pi)^d}
\frac{1}{(k^2+m^2)^2}}_{\cali}
\eeq
...
\left(
\frac{1}{\eps}  
-\ln\left(\frac{m}{\mu_E}\right)
\right) + \calo(\eps)
\eeq
\beq
\mu_E = 2\sqrt{e^{-\gamma_E}\pi}
\eeq
...
\beq
\left(\frac{m^2}{4\pi}\right)^{-\frac{\eps}{2}}
= 1 -\frac{\eps}{2}\ln\left(\frac{m^2}{4\pi}\right)+\calo(\eps^2)
\eeq
...
\beq
0=\partial_\lam V(\lam^*)=-\beta(\lam_*)
\eeq
...
\section{tempo}
 ...
 2. The diagram: 1-loop 4-point function

There are **3 identical “fish” diagrams** (s, t, u channels).

Each gives:
$$
\sim \lambda^2 \mu^{2\epsilon} I(p)
$$

We compute one and multiply by 3.
...
\left[
\frac{1}{\eps} 
-\ln\left(\frac{\mu}{M\sqrt{4\pi}}
\right)
\right]
\eeq
with
\beq
M^2 = \exp\left(\int_0^1dx\; \ln\Delta\right)
\eeq
\beq\Delta = m^2 + x(1-x)p^2
\eeq
\end{claim}
\proof
\qed
...
So:
$$
I(s) = \int_0^1 dx \int \frac{d^d k}{(2\pi)^d}
...
---
 ...
 4. Dimensional regularization integral
...
$$
\Delta^{-\epsilon/2}
= e^{-(\epsilon/2)\ln \Delta}
= 1 - \frac{\epsilon}{2}\ln \Delta + O(\epsilon^2)
$$
...
(4\pi)^{-2 + \epsilon/2}
= \frac{1}{(4\pi)^2}
\left(1 + \frac{\epsilon}{2}\ln 4\pi \right)
$$
