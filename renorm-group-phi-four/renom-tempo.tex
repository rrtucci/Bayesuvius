...
coarse-graining produces   nontrivial -fluctuations  , and those produce a genuine $ \phi^4 $ interaction.
 3 Why $d=4 $ in $ \phi^4 $ vs $d=2 $ Ising

This is the subtle but crucial part.

 Short answer

  Ising model exists in any dimension 2D, 3D,
  $ \phi^4 $ has a   special dimension $d=4 $  :
...
In:
 $$
S = \int d^d x \left[
\frac{1}{2}(\nabla\phi)^2 + \frac{u}{4!}\phi^4
\right]
 $$

Demand action dimensionless.
...
---
 ...
 Interpretation
  ...
  $ d < 4 $:
  $ u $ has   positive dimension   to   relevant
  $ d = 4 $:
  $ u $ is   dimensionless   to   marginal
  $ d > 4 $:
  $ u $ is   irrelevant
...
---
 ...
 Physical meaning
    ...
    Below 4D  :
  fluctuations matter to nontrivial critical behavior
    Above 4D  :
  fluctuations die to mean-field theory works

---

 So why 2D Ising works with $ \phi^4 $?

Because:

  2D < 4 implies quartic interaction is   strongly relevant  
  RG flows to a   nontrivial interacting fixed point  

So:

> $ \phi^4 $ in $ d=2 $ flows to the   2D Ising CFT

---

 Big picture

  $ d=4 $ is   not the dimension of Ising
  it is the dimension where $ \phi^4 $ becomes marginal

You can study:

  Ising in 2D iff $ \phi^4 $ in 2D
  Ising in 3D iff $ \phi^4 $ in 3D

But:

  calculations are easiest near $ d=4 $ to $ \epsilon = 4-d $

---

 Why people expand around $d=4 $

Because:

  theory is weakly coupled near $ d=4 $
...
---
 ...
 4 Clean dictionary (final)

| Ising                     | $ \phi^4 $               |
| ------------------------- | ------------------------ |
| $ \beta J - (\beta J)_c $ | $ r $                    |
| $ \beta H $               | $ h $                    |
| magnetization $ m $       | $ \langle \phi \rangle $ |
| spin symmetry             | $ \phi \to -\phi $       |
| dimension $ d $           | same $ d $               |

---

 5 One-line intuition

  Ising =   microscopic lattice spins  
  $ \phi^4 $ =   coarse-grained continuum field
  $ d=4 $ =   where fluctuations start to matter

---

If you want, I can next:

  explicitly compute how $ r \sim T - T_c $ emerges from HS
  or show the 1-loop RG flow of $ u $ and how the Wilson–Fisher fixed point appears (this ties directly to your anomalous-dimension work)
