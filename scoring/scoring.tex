...
\\
&=&
\ln \prod_\sigma P(x^\s\mid \theta)
\\
&=&
\sum_\sigma \ln P(x^\s\mid \theta)
\\
&\approx&
...
\hrule\noindent
Dir$(\cdot)$ is {\bf conjugate prior} of  Mul$(\cdot)$
...
\eeq
\hrule\noindent
Dir$(\cdot)$ is replaceable by a Mul$(\cdot)$
...
for the expected value of Dir$(\cdot)$,
we get
