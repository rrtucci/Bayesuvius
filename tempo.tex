...
\ul{dx^i}(\vecX)
\wedge \ul{dx^j}(\vecY)=
\ul{dx^i}(\vecX)\ul{dx^j}(\vecY)
-
\ul{dx^i}(\vecY)\ul{dx^j}(\vecX)
\eeq
for $i, j\in\ZZ_1^n$.
Hence, 
...
\ul{\times dx^I}(\vecX^{(\s(I))})
=
\ul{\times dx^I}(\vecX^{UA(I)})
\eeq
for $I \in \cali^p$,
...
\\
&=&
\ul{dx}(\vecX)\ul{dy}(\vecY)
-
\ul{dy}(\vecX)\ul{dx}(\vecY)
\\
&=&
X^1 \ul{dy}(\vecY)
- X^2 \ul{dx}(\vecY)
...
\item {\bf Lie derivative} $\call_\vecX$ gives
