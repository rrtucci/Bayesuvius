...
\newcommand{\vecN}[0]{{\vec{N}}}
\newcommand{\vecT}[0]{{\vec{T}}}
\newcommand{\vecU}[0]{{\vec{U}}}
\newcommand{\vecV}[0]{{\vec{V}}}
\newcommand{\vecX}[0]{{\vec{X}}}
\newcommand{\vecY}[0]{{\vec{Y}}}
\newcommand{\vecZ}[0]{{\vec{Z}}}


\newcommand{\vece}[0]{{\vec{e}}}
...
$\{\vec{e}_i\mid i\in \ZZ_1^n\}=$ {\bf tangent basis}, a basis for $T_\pi(M)$. These vectors need not be orthogonal nor normalized.

$\{\check{x}_i\mid i\in \ZZ_1^n\}=$ {\bf coordinate (tangent) basis}
for coordinates
$\{x_i\}_{i=1}^n$ , a special tangent basis for $T_\pi(M)$.
Unlike the generic tangent basis
$\{\vec{e}_i\}_{i=1}^n$, a coordinate tangent basis
...
for which differentiable functions $f(x_1, x_2, \cdots, x_n)$ exist.
 As with the basis  $\{\vec{e}_i\}_{i=0}^n$, the  basis $\{\check{x}_i\}_{i=0}^n$ need not be orthogonal nor normalized.
...
A p-form $\ul{\omega}$ is also defined pointwise. When we write
$\ul{\omega}(\vecX, \vecY)$,
...
$T(M)$ ha dimension $2n$.
\hrule
Historical sidebar on the term \qt{holonomic}

Early workers in this field
refer to 
a coordinate basis as a {\b holonomic} basis.
The word \qt{holonomic}
comes from the Greek
words \qt{holos}=whole, complete
and \qt{nomos}=a rule 
or prescription.
...
In differential geometry, the term holonomic refers to a basis of vectors 
$\{\vece_i\}_{i=0}^n$
...
for all $i\in\ZZ_1^n$.
In conclusion,
a coordinate basis and
a holonomic basis are
the same thing.

A tangent basis $\{\vece_i\}_{i=1}^n$ is a coordinate=holonomic
basis iff 
\beq
[(\vece_i)_{op},
(\vece_j)_{op}]=0
...
$(\hat{r}, \hat{\theta})$ are non-holonomic because
...
(vierbein) basis. That tangent basis is non-holonomic too.
...
They are called the {\bf coordinate dual basis} \footnote{
A basis is often called a {\bf frame} and a dual basis is called a {\bf coframe}. We won't use that nomenclature.} for coordinates 
$\{x_i\}_{i=1}^n$.
 They satisfy
...
\cellcolor[HTML]{FFFFFF} &  basis & dual basis & \begin{tabular}[c]{@{}l@{}}displacement \\ vector\end{tabular} &  \\ \hline
\cellcolor[HTML]{DAE8FC}coordinate basis & $\check{x}_a$ & $\pform{dx^a}{}$ & $\pform{d\vecr}{}= \check{x}_a\pform{dx^a}{}$ & $\ul{dx^a}(\check{x}_b)=\delta^a_b$ \\ \hline
\cellcolor[HTML]{DAE8FC}non-holonomic basis & $\vec{e}_a$ & $\pform{\theta^a}{}$ & $\pform{d\vecr}{}=\vece_a\pform{\theta^a}{}$ & $\ul{\theta^a}(\vece_b)=\delta^a_b$ \\ \hline
\end{tabular}
\caption{Notation for coordinate (holonomic) and non-holonomic basis.}
...
\ul{\theta^n}
$
is an {\bf oriented orthonormal dual basis}
Then
\beq
\text{vol}=
\ul{\theta^1}\wedge\cdots\wedge \ul{\theta^n}
\eeq
...
\beq
\underbrace{\int\int\cdots\int}_{\text{$p+1$ times}} d \pform{\omega}{p}
=
\underbrace{\int\int\cdots\int}_{\text{$p$ times}}\pform{\omega}{p}
\eeq
...
basis, $\hat{n}$ normal to tangent plane

$(\pform{\theta^1}{}, \pform{\theta^2}{})=$ dual basis. It satisfies
...
\beq
\pform{d\vecr}{} = \hat{e}_a\pform{\theta^a}{}\quad\text{(displacement vector)}
\eeq
...
\beq
\pform{d\hat{e}_b}{} =
\pform{\omega\indices{^b_a}}{}\hat{e}_b
\label{eq-de-w-e}
\eeq
...
&\quad  \text{(First SE, $\ul{T^a}=$ Torsion)}
\\
...
& \quad \text{(Second SE, $\ul{\Omega\indices{^a_b}}=$ Curvature)}
\end{align}
For Euclidean space ($\RR^3$), we don't have
to distinguish between upper and lower indices.

Eq.(\ref{eq-de-w-e}) expressed in matrix form is 
...
$[\ul{\omega_{ab}}]$ must be anti-symmetric because the basis $(\hat{e}_1, \hat{e}_2)$
is orthonormal.
...
Instead of using the coordinate basis
...
p-forms 
and tetrad basis (Cartan formulation)
...
$\pform{\theta^a}{}$ called the {\bf dual tetrad (vierbein) dual basis} for
...
Tensor Formulation & \cellcolor[HTML]{FFFC9E}Cartan (p-forms, tetrad basis) formulation \\ \hline
