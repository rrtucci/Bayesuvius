...
\begin{figure}[h!]
 ...
 As show in Fig.\ref{fig-hysterisis-2-saddle-point}, {\bf hysterisis}
 arises in systems with 2 saddle points. In the $(\lam, x)$ plot,
 as the parameter $\lam$ is 
 cycled between $-\lam_c$ and $\lam_c$,
 the system travels along a
 line of increasingly weak attractors which turn into  saddle point 1, at which point the system jumps to a string of increasingly weak attractors
 that turn into saddle  point 2.

 \begin{figure}[h!]
\centering
\includegraphics[width=5in]
{dynamical-sys/hysterisis.png}
\caption{In hysterisis,
as we cycle 
$\lam$ between $\lam_\ell$ aand $\lam_h$, the $x(t)$ trace is a
train of pulses.}
\label{fig-hysterisis}
\end{figure}

 As shown in Fig.\ref{fig-hysterisis},
 if we cycle the parameter $\lam$
 between $-\lam_c$ and 
 $\lam_c$, the $x(t)$ trace is a
 train of pulses
...
A {\bf limit cycle} is a closed trajectory with no saddle points
 in it. There are four types of limit cycles, depending on whether their skin attracts or repels inside and outside.
 These 4 types are portrayed  in Fig.\ref{fig-limit-cycles}. Of these 4 types, only one type
 is truly stable,  the one whose skin attracts both inside and outside (i.e., type $(A)A$).
...
skin of the cycle attracts inside and repels outside,
...
In limit cycles, the skin of a closed trajectory attracts
or
repels inside or outside, leading to spiraling behavior inside or outside. It's also possible for
the skin of a closed trajectory to neither attract nor repel the flow. The flow
simply runs parallel to the skin, either inside or outside the skin, or both.


As illustrated by Fig.\ref{fig-homo-hetero-clinic}, 
a {\bf homoclinic (or heteroclinic) orbit}  is a closed trajectory which contains a one saddle point if homoclinic (or two if heteroclinic), and for which the flow runs
parallel to the skin, both inside and outside. 
A trajectory can enter or exit the orbit through any of the saddle points of the orbit.
...
for very long time because any system has finite resources
...
\beq \ddot{x}  +\mu^2(1-x^2)\dot{x}+ x=0\eeq
 so $2\gamma = \mu^2$, $V=-\mu^2$
and $\omega^2_0=1$).
...
($V=0$, $R\neq 0$) and hybrid  ($V,R\neq 0$)
oscillators.
Note that the V and R oscillators have peaks at higher order 
odd harmonics ($3\omega_0, 5\omega_0$, etc.)
whereas those higher order harmonics can be
suppressed in a hybrid oscillator with the right 
ration of $V$ to $R$.
Of course, the linear harmonic
oscillator
...
The  {\bf Lorenz system} arises in fluid mechanics, laser physics, and other fields.
It is given by
