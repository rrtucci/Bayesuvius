...
\beq
\ddot{x} + 2\gamma \dot{x} + \omega^2 x= F_0\sin(\omega t)
\eeq
where $\gamma, \omega\in\RR$. The right hand
side term is called the {\bf forcing term}. We will
set $F_0=0$ for this discussion.
...
\dot{x} = v
\\
\dot{v} = -2\gamma v -\omega^2 x
\end{array}
\right.
\eeq

\beq
x = \Re\left(Ae^{i\delta}e^{\lam t}\right)
\eeq
where $A, \delta\in \RR$ and $\lam\in \CC$

\beq
A(\lam^2 + 2\gamma\lam +\omega^2) = 0
\eeq

\beqa
\lam &=&\frac{1}{2}(-2\gamma \pm \sqrt{4\gamma^2-4\omega^2})
\\
&=&
-\gamma\pm\underbrace{\sqrt{\gamma^2-\omega^2}}_{\Omega}
\eeqa

\beq
x =\Re(Ae^{-\gamma t +i(\Omega t +\delta)})
\eeq
...
\beq 
\calf_3 = \underbrace{
\underbrace{P x^2\dot{x}}_{\text{Van der Pol term}}
+
\underbrace{R \dot{x}^3}_{\text{Raleigh term}}
}_{\text{hybrid term (odd in $\dot{x}$)}}
+ \underbrace{ a x(\dot{x})^2  + b x^3}_{\text{even in $\dot{x}$}}
\eeq
...
\caption{Hybrid oscillator. This
 figure  comes from Ref.\cite{dynamical-fuchs}.}
