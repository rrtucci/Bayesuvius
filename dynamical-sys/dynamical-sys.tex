...
Let\footnote{A system of ODEs such as
$
\left\{
\begin{array}{l}
\dot{x}= ax + by
...
but delta bnets are 
not very explicit when expressing
the structure of 
ODEs. They seem better suited
for expressing the structure of
PDEs (such as one finds in
discussing Thermodynamics) instead of the structure of ODEs (such as one finds in discussing Dynamical Systems).}
...
Let $\mu = \mu_R + i\mu_I$. Note that in the linear approximation which is valid for $|\vec{x}|\ll 1$, we have
$\dot{\vec{x}}=A\vec{x}$.
...
where $0<\epsilon \ll 1$, this reduces  to
