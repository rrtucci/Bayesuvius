...
\label{eq-linear-nn}
\eeq
Activation functions  $\phi^\lam_i:\RR\rarrow \RR$
for NNs
...
$\phi^\lam_{i, BN}(\cdot)$ by
\beq
\phi^\lam_{i, BN}(z^{\lam, \s}_i)=
\gamma^\lam_i
\left[
...
If node $\rvh^\lam_i$ of the NN has activation function
 $\phi^\lam_i:\RR\rarrow \RR$,
defined a new activation function $\phi^\lam_{i, new}:\RR\rarrow \RR$ by
the composition of functions
\beq
\phi^\lam_{i, new}
 =\phi^\lam_i\circ\phi^\lam_{i, BN}
\eeq
Hence, the BN activation
function
is applied after the linear transformation
Eq.(\ref{eq-linear-nn}), but before the nonlinear
transformation $\phi^\lam_i$.
