...
TNs have many
advantages for doing NLP
compared to the older method of RNNs.
Whereas RNNs
analyze the
tokens (words)
of
...
be the  {\bf prior-weight matrices for time
slice $t$}.
...
\beq\color{blue}
P(v^t_i|V^t, h^t_i)
=\indi(\;\;\; v^t_i=V^t h^t_i
\;\;\;)
\eeq

\beq\color{blue}
P(q^t_i|Q^t, h^t_i)
=\indi(\;\;\; q^t_i=Q^t h^t_i
\;\;\;)
\eeq

\beq\color{blue}
P(k^t_i|K^t, h^t_i)
=\indi(\;\;\; k^t_i=K^t h^t_i
\;\;\;)
\eeq
...
it is more common to use instead
