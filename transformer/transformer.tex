...
\qt{Attention is all you need} paper,  order the
...
The right side of Eq.(\ref{eq-attention-average})
...
is the original
one from the \qt{Attention is all
you need paper}. Some people
call it the \qt{scaled dot product Attention}.}
...
Fig.\ref{fig-transformer-full3} are each \qt{shrunk} to single nodes,
...
be \qt{folded} (i.e., the 3 words can be
...
Note that Attention$(\cdot)$ always has the
...
Important: Note that the softmax$(\cdot)$ makes the
...
softmax$(\cdot)$ takes a vector and returns
...
are the two input signals and \qt{norm} is some norm of $A+B$ (for
instance, $\norm{A+B}_2$).
...
\qt{Outputs (Shifted Right)} in Fig.\ref{fig-vanilla-transformer}
...
$\alp$, and $\alp'$ are
word positions in a
sentence, and $\alp'$ is in the future (downstream)
compared to $\alp$.
So as to
not violate causality,
this condition enforces the constraint that no attention is paid to word positions in the future of $\alp$.
...
\subsection{BERT parameter values}
...
\end{tabular}
\caption{Some hyperparameter values for BERT base and BERT large}
\label{tab-bert}
\end{table}
...
Let's consider a short sentence: \qt{The cat is on the mat.}
...
\begin{enumerate}
  ...
  \item \textbf{Tokenization into words:}
       ...
       \qt{The}, \qt{cat}, \qt{is}, \qt{on}, \qt{the}, \qt{mat}, \qt{.}
  ...
  \item \textbf{Adding Special Tokens:}
     Add special tokens, such as [CLS] (classification) at the beginning and [SEP] (separator) at the end:
      ...
      \qt{[CLS]}, \qt{The}, \qt{ca}, \qt{t}, \qt{is}, \qt{on}, \qt{the}, \qt{mat}, \qt{.}, \qt{[SEP]}
...
this is called \qt{pre-training}
to distinguish it from
the \qt{training} of the  smaller NN that is attached to the output of
BERT when doing
...
\begin{itemize}
\item Given \qt{[CLS] my dog is cute [SEP] he likes playing}, the model
should output token [IsNext].
\item Given \qt{[CLS] my dog is cute [SEP] how do magnets work}, the model
