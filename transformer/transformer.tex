...
in a sentence.
This average is called a {\bf Attention}
(for a single head).
...
On first encounter, the structure of a tranet
...
$D= n_\rvh d$,
where $d$
is, as before, the hidden, embedding dimension. $\Delta \in [D]$. We will do some tensor reshaping 
...
\caption{Multi-head version of
the single head tranet Fig.\ref{fig-transformer-recurrent-folded-1head}. Note that all orange nodes
have the same tensor shape.}
\label{fig-transformer-recurrent-folded-multi-head}
\end{figure}

The structure equations, printed in blue,
for the bnet 
Fig.\ref{fig-transformer-recurrent-folded-multi-head}, are as follows.
Note that Attention always has the
same shape as its 3 arguments. Note also that the
3 weight matrices
$W_\rvk^{[D], [d]}$,
$W_\rvq^{[D], [d]}$,
and
...
\end{minipage}
\caption{Encoder of Vanilla Transform Net.}
