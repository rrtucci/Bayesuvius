...
\beq\color{blue}
P(s_t[i]\mid s_{t-1}[i], a_{t-1}[i]) = \text{ given}
\eeq

\beq\color{blue}
P(a_t[i]\mid s_t[i], \theta_t)= \text{ given}
\eeq

\beq\color{blue}
P(r_t[i]\mid s_t[i],a_t[i])=
\delta(r_t[i],r(s_t[i], a_t[i]))
\eeq
$r:val(\rvs)\times val(\rva)\rarrow\RR $ is given.

\beq\color{blue}
P(\Delta v_t[i]\mid s_t[i], a_t[i], s_{t+1}[i])=
