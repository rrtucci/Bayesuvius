...
\left[
\begin{array}{c}
\pi_{ \rvx_i|\ul{cx}_i}(\cdot)
\\
\pi_{\ul{cx}_i| \rvx_i}(\cdot)
\end{array}
\right]
\;,
\;\;\;
...
\left[
\begin{array}{c}
\pi_{ \ul{px|\rvx_i }_i}(\cdot)
\\
\pi_{\rvx_i | \ul{px}_i}(\cdot)
\end{array}
\right]
\;.
\eeq
...
B
=
\underbrace{\rvx|P(\pi_{\rvb}|
\pi_{\rva|\rvx})}_{B_\pi}
\underbrace{P(\pi_{\rvx| \rva}|
\pi_{\rvb| \rvx})}_{B_\pi}
\;,
\eeq
where
...
B_\pi=
\prod_{x}\indi\left(
\underbrace{\pi^{(t)}_{\rvx|\rvb}(x)}_
{OUT}
=\sum_a P(x|a)
\underbrace{\pi^{(t)}_{\rva|\rvx}(a)}_{IN}
\right)
\eeq
and
...
\prod_{a}
\indi\left(
\underbrace{\pi^{(t)}_{\rvx| \rva}(a)}_{OUT}
=
\sum_x P(x|a)
\underbrace{\pi^{(t)}_{\rvb| \rvx}(x)}_{IN}
\right)
\;.
\eeq
...
\beq
\underbrace{\pi_{\rvx|\rvb_i}
(x)}_{OUT}=
\caln(!x)
\underbrace{\pi_\rvx(x)}_{IN}
\prod_{k\neq i}
\underbrace{\pi_{\rvb_k| \rvx}(x)}_{IN}
\label{eq-mp-rule2}
\eeq
(boundary case:
...
\beq
\underbrace{m_{\alp| 2}(x_2)}_{\pi}
=
\sum_{x_1}
P(x_2|x_1)
\underbrace{ m_{1|\alp}(x_1)}_
{\pi}
\;.
