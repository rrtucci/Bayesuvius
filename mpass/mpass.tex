...
type of message passing (MP).
In the next section,
we will
generalize this MP for soldiers
to MP for a Markov chain.
\newpage
...
Before 
concluding our
discussion of MP for Markov chains,
let us consider MP
...
In analogy
with the case of MP for a 3 node Markov
...
\caption{MP 2-track bnet for the bnet
...
\caption{MP Memory Bnet for the bnet
Fig.\ref{fig-mp-5chain}. }
...
The function $\calc$,
which 
we will call the {\bf MP local computation},
...
\caption{MP dynamic bnet for the bnet
...
\section*{MP Algorithm for Polytrees}
...
\hrule\noindent{\bf How MP algo
for polytrees reduces to 
MP algo for Markov chains}
...
\section*{Derivation of MP Algorithm
for Polytrees}
...
The MP algorithm yields an expansion
 for $P(x|\eps)$.
...
Next we prove MP rules 1 and 2.
...
used in the derivation of the MP
RULE 1.}
...
\section*{Example of MP algo for a  Tree}
...
\item {\bf
We find $m_{\alp\ldart i}^{(t)}$
 as follows.}

See
the
$m_{\rvf_2\ldart\rvx_1 }$ panel of
...
\item {\bf
We find $m_{\alp\rdart i}^{(t)}$ as follows.}

See the
$m_{\rvf_1\rdart \rvx_2}$ panel
...
\\
&=&
\eps^{(t-1)}_{(x_k)_{k\in nb(\alpha)-i}}[
f_\alpha(x_{nb(\alpha)})]
\;,
\label{eq-mp-iter2}
\eeqa
...
\beq\color{blue}
P(m^{(t)}_{\alp\rdart i}(\cdot)\cond
pa[m^{(t)}_{\alp\rdart i}(\cdot)]
)=
\indi(\;\;\;
m^{(t)}_{\alp\rdart i}(x_i)
=
...
\beq\color{blue}
P(
m^{(t)}_{\alp\ldart i }(\cdot)
\cond
pa[m^{(t)}_{\alp\ldart i }(\cdot)]
)
=
\indi(\;\;\;
m^{(t)}_{\alp\ldart i }(x_i)
...
\rvA\ar[d]\ar[rrd]
\\
m^{(1)}_{\ul{P_{\rvA_0|\rvA}}\ldart\rvA}\ar[d]
&&
m^{(1)}_{\ul{P_{\rvA_1|\rvA}}\ldart\rvA}\ar[d]
\\
m^{(2)}_{ \ul{P_{\rvA_0|\rvA}}\rdart\rvA_0}
\ar[d]\ar[dr]
&&
m^{(2)}_{ \ul{P_{\rvA_1|\rvA}}\rdart\rvA_1}
\\
m^{(3)}_{
\ul{P_{\rvA_{00}|\rvA_0}}\ldart \rvA_0}\ar[d]
&
m^{(3)}_{
\ul{P_{\rvA_{01}|\rvA_0}}\ldart \rvA_0}\ar[d]
\\
m^{(4)}_{
 \ul{P_{\rvA_{00}|\rvA_0}}\rdart\rvA_{00}}
&m^{(4)}_{
 \ul{P_{\rvA_{01}|\rvA_0}}\rdart\rvA_{00}}
}
$$
...
P(A_{00})=
\caln(!A_{00})
m_{ \ul{P_{\rvA_{00}|\rvA_0}}\rdart\rvA_{00}}(A_{00})
\eeq
...
\sum_{A_0}
P(A_{00}|A_0)
m_{\ul{P_{\rvA_{00}|\rvA_0}}\ldart \rvA_0}(A_0)
\eeq

\beq
m_{\ul{P_{\rvA_{00}|\rvA_0}}\ldart \rvA_0}(A_0)
=
m_{\ul{P_{\rvA_{0}|\rvA}}\rdart\rvA_0 }(A_0)
\eeq

\beq
m_{\ul{P_{\rvA_{0}|\rvA}}\rdart \rvA_0}(A_0)=
\sum_{A}P(A_0|A)
m_{\ul{P_{\rvA_{0}|\rvA}}\ldart \rvA}(A)
\eeq
