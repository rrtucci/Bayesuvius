...
(but in reality don't, these are just a convenient
...
\hrule
{\bf Message Arrows Notation} (See Fig.\ref{fig-mp-1chain}):
...
of the double arrow. It's also true that
in both $\pi_{\rvb\ldart\rva}(a)$ and
$\pi_{\rvb\ldart\rva}(a)$, the argument $a$ in parentheses
appears next to $\rva$ in the subscript.
...
which illustrates
a bnet node $\rvx$ receiving messages from and sending
them to its neighbors.
The $\pi$ and $\lam$ messages
are labeled in accordance to
the notational conventions
previously stated in Section \ref{sec-bp-markov}.
The $\pi$ messages
(probability functions)
travel downstream (i.e., 
they carry info
...
\caption{Node $\rvx$ receiving messages from
and sending them to
...
&=&
P(\eps^+_\rvx\cup \eps^-_\rvx) - \eps^-_{\rvx \rva_i}|a_i)
\\
&=&
P(\eps^+_{\rvx \rva_i}|a_i)
\eeqa
...
&=&
P(x|(\eps^+_\rvx\cup \eps^-_\rvx) - \eps^+_{\rvx \rvb_i})
\\
&=&
P(x|\eps^-_{\rvx \rvb_i})
\eeqa
...
\eeq
One can rewrite the
left and right-hand sides (LHS, RHS)
...
x_3
&\sum x_2\ar[l]
&\sum x_1\ar[l]\ar@/_1pc/[ll]
&\EE x_0\ar[l]\ar@/_2pc/[ll]\ar@/_2pc/[lll]
}\ecen
\eeqa
