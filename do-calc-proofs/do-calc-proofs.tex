...
\section{Promise Keeping Operations}

So far, we have defined
a {\bf promise} to be the full
probability distribution of a bnet.
Next, we shall introduce
the concept of {\bf original promise}. Suppose
...
\beq
P(z, y, x)= P(z|y)P(y|z)P(z)
\eeq

\item the original Promise $\pi_b$ equals:

\beq
Q(z, y, x)=Q(z|y, x) Q(y\mid x)Q(x) 
\eeq
\end{itemize}
...
a sum over intermediate nodes such as $\rvy$, 
removes all
trace of the OP.
This does not cause problems
as long as we are dealing
with just one OP.
However, in doing Do Calculus,
we deal with more than one OP
at a time,
and in that case, not distinguishing between OPs
can lead to erroneous conclusions. A way to 
avoid this pitfall is to 
represent instantiations of
a bnet with diagrams, and
to use only 
 promise keeping (PK) operations
on that diagram.
We say an {\bf operation is PK} if it does not erase some
trace or memory of the OP.
...
follow from  the
following 2  
{\bf basic promise-keeping (PK)
operations}:
\begin{enumerate}
\item
If one has a fully connected exploding clique, one is free to
...
be the bnet $G$ conditioned on $\rvx=x$. If $G$ has promise $P_G(\cdot)$,
...
a promise bnet $G_{promise}$.
Then for a query
$P(y|\cald\rvx=x)$, we express the adjustment
formula in terms of the promise $P(\cdot)=P_{G_{promise}|x}(\cdot)=P_{G_{promise}}
(\cdot|x)$.
