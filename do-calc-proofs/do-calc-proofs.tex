...
for bnets
with or bnets without hidden nodes:\footnote{Note that $P_{G_x}(y|x)\neq P_G(y|x)=P(y|x)$.
In fact,
$P_{G_x}(y|x)= P(y|x)$
...
The following two step heuristic technique often
yields a valid adjustment formula.
...
are summed over.
Summation
over nodes can be left explicit,
to suggest how to compute
the adjustment formula, or
they can be removed for brevity (as long as we
remember the original bnet whence they came from.)



\item If $G$ has hidden nodes,
these must be summed over
and replaced by observed TPMs via the node 
...
is not a valid
adjustment formula
because, for this bnet, 
try as we may,  there
is no way to replace the sum over
hidden  node $z$ by an
equivalent sum over an observed node.
