...
\item Remove all sums $\sum h$ over hidden nodes $\rvh$. This is justified by the node summation 
identities.

\item 
Replace every average $\EE h$ over a
hidden node $\rvh$ with an $\EE a$,
where $\rva$ is any observed node. 
This step does not always lead to a valid adjustment formula.
If it doesn't,
