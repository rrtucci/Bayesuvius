...
\item The original promise $\pi_a$ equals:
...
\item the original Promise $\pi_b$ equals:
...
OP whereas operation
Eq.(\ref{eq-universal-op})
is a {\bf universal operation}, in the sense that it is valid for any OP.


Often, people (including me) do not write
the conditioning on the OP
or use separate symbols $P(\cdot)$ and $Q(\cdot)$.
This removes memory of the OP.
This does not cause problems
as long as we are dealing
with just one OP.
It's also common to
use non-universal operations
to simplify an instantiation.
This removes memory of
how the instantiation was built (modulus universal operations which are not recorded). A way to 
avoid this pitfall is to 
...
of the arrows to any 
other directions,
as  long as the 
final clique is acyclic.
\item
