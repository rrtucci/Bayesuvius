...
$\delta P=0$ implies $\delta\theta=0$.} Equivalently, {\bf $\theta$ is not
identifiable} 
 iff
...
For Pearl, a {\bf causal effect is identifiable} (and the hidden variables
corresponding to $\theta$  
are not identifiable) for a promise $P$ iff 
\beq
\exists \text{ changes in the 
hidden variables such that}\quad\quad
\delta P=0
\eeq
I'm going to adopt the stronger definition: a {\bf causal effect is identifiable} (and the hidden variables
corresponding to $\theta$  
are not identifiable) for a promise $P$ iff 
\beq
...
\delta P=0
\eeq
(i.e., the hidden variables
are totally, not partially, hidden.)



Let's be more specific about
the causal effect case.

Given a probability distribution $P(x)$, we
define a {\bf zero-sum variation}
$\delta P(x)$ of $P(x)$ so that 

\beq 
1=\sum_x P(x)=\sum_x\left[P(x)+\delta P(x)\right]\implies \sum_x \delta P(x)=0
\eeq
...
corresponds to the
instantiation of $\cald_\rvx \calg$ wherein all nodes other than $x$ and $y$ have been 
...
\section{Promise Independent Node Summation (NS) Identities}
Next we will
give some {\bf node summation  (NS) identities}
...
The moral of the above NS identities is that
if one has a fully connected bnet, or a bnet clique
(i.e., fully connected subgraph of a bnet)\footnote{Note that in order to use the NS identities on a clique,
...
& \sum x'
\\
x\ar[r]\ar[rru]\ar[ur]|0
&\sum m\ar[r]
&y\ar@[red]@{<-}[u]
}
\ecen
\begin{array}{l}
\text{(fully connected 4 node graph so we} 
\\
\text{are allowed to reverse red arrows)}
\end{array}
\\
&=
\bcen
...
because you it's easy to find
a zero-sum variation $\delta P(u_1)$
such that
