...
\item{\bf Auto-correlation (ACorr)} (assuming
w-stationarity)
...
{\bf external noise} or
{\bf shock} or {\bf innovation}
at time $t$.
...
\sum_{j'=-\infty}^\infty\nu_j\nu_{j'}
\underbrace{\av{
\rvn_{-j}, \rvn_{\tau-j'}}}_{\delta(j', \tau + j)}
\\
&=&
\sum_{\tau=-\infty}^\infty z^\tau
\sum_{j=-\infty}^\infty\nu_j\nu_{j+\tau}
...
Also,
the opposite is true: $\rvy_{\leq t}$
can be expressed in terms
of $\rvn_{\leq t}$,
yielding a function 
...
\beq
\rvy_{t+\tau}|\rvy_{\leq t}=
\rvy_{t+\tau}|\rvn_{\leq t}(\rvy_{\leq t})
\eeq
...
of $\alp^-(\calb)$, then,
in order 
to forecast $\rvy_{t+\tau}|\rvt_{\leq t}$,
we have the option
...
\beq
\rvy_t = \underbrace{\sum_{j=0}^\infty
(\alp_1\calb)^j}
_{\nu(\calb)}
\rvn_t
\eeq
...
\rvy_{\leq \tau}
&\approx&
\left[\call_{WK}\right]_
{\calb^{>m}}
\rvy_t\indi(\rvy_{\leq t-m}=0)
\eeqa
...
to $\rvs_t$
decrease geometrically (exponentially) as the lag
$j$ increases. 
...
\exp\left[
\frac{\left(y_t-\sum_{j=1}^p\alp_jy_{t-j}\right)^2}
{2\s^2}
\right]
\right]P(y_{\leq p}|\theta)
\end{align}
...
\beq
\{(\tau, y_{[\tau-p, \tau-1]}, \boxed{y_\tau}):
 \tau\in\ZZ_{[p+1, t]}\}
\eeq
This dataset is strange because
the typical dataset that we use to 
do LR is assumed to satisfy the i.i.d.
assumption,
