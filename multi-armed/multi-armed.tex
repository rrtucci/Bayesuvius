...
and $N_{-1}(a)=N_{in}$
for all $a$.
We assume that at large $t$, $N_{in}\ll N_t(a)$
for all $a$.
...
and $\HAT{P}(r_t|a_t)$,
where $\HAT{P}(\cdot)$
is an empirical estimate of $P(\cdot)$.}
...
$\caln(\cdot)$ stands for the Normal distribution
and Gamma$(\cdot)$ for the Gamma distribution.
