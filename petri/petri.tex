...
$$
\caption{Same pnet as that in Fig.\ref{fig-original-pnet-eg-square},
except that here
...
if there is an arrow $\rvp_i\rarrow \rvx_j$
(resp., $\rvp_i\larrow \rvx_j$).
...
\right]
\eeq
If we fire transition  $\rvx_2$ only (i.e., if we set $x_1=0, x_2=1$),
we get 
...
See Fig.\ref{fig-petrify-ar-matrix-messages}. Inspired by Pearl's message passing 
