...
By (conventional) {\bf firing step} of a transition $\rvx$, we mean the action of removing $\mu(\rvp\rarrow \rvx)$
...
Given a bnet, one can construct a natural pnet, call it the bnets {\bf Bayes Petri Net},  that uses the nodes of the bnet as the transition nodes of the Bayes pnet. We refer to this construction
 process as {\bf petrifying a bnet}.
 
 Inspired by Pearl's message passing 
 theory (see Chapter \ref{ch-mpass}), we petrify a single arrow bnet
 by adding to it two pnet arcs: (see Fig.\ref{fig-petrify-probs-messages} )
 \begin{itemize}
 ...
 $$
 \caption{Petrifying an arrow with matrix messages.}
 \label{fig-petrify-probs-messages}
 \end{figure}
