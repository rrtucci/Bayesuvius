...
tokens from all $\rvp$ such that $\rvp\in\rvx^\larrow$, and adding $\mu(\rvx\rarrow\rvp')$ tokens to all $\rvp'$
such that $\rvp'\in\rvx^\rarrow$.

A transition $\rvx$ is {\bf enabled} 
(i.e., it may fire)
for $\rvp\in \rvx^{\larrow}$, if there are enough tokens in 
$\rvp$ for a firing to be possible; i.e., if $M(\rvp) \geq \mu(\rvp\rarrow \rvx)$. A transition $\rvx$ is {\bf fully enabled} if it is enabled for all $\rvp\in \rvx^\larrow$.
...
\end{array}
\eeq
The extended matriix $W^{new}$ is now
...
store their relative number.
In the conventional case,
the messages are scalars too, 
but in the multicolored case,
they become matrices or even tensors.
In fact,  one can generalize conventional Petri networks
