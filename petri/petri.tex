...
\caption{Simple example of an original pnet. The number of tokens in each place
...
$\cala =$  the set of all arrows.
 \beq\cala \subset \ol {\cala} = (\calp \times \calx) \cup (\calx \times \calp) = \{\calp\rarrow\calx \text{ and } \calx \rarrow \calp:
...
\hrule
$\mu : \calp \rarrow \ZZ_{\geq 0}=$ {\bf markings (i.e., contents) of  each place}. Sometimes, it is convenient to consider a fractional number of tokens in a place, so
...
By a {\bf firing step} of a transition $\rvx$, we mean the action of removing $\kappa(\rvp\rarrow \rvx)$
...
$\rvp$ for a firing to be possible; i.e., if $\mu(\rvp) \geq \kappa(\rvp\rarrow \rvx)$. A transition $\rvx$ is {\bf fully enabled} if it is enabled for all $\rvp\in \calp(\rvx\larrow)$.
...
\eeq
where $W(t)=0$ if 
transition vector $\ket{a(t)}_\calx$ 
...
This leads
to the definition of a {\bf colored pnet}, wherein
place node markings
