...
(i.e., $f''(x)<0$
if differentiable).
More generally, if $f:\cald_f\rarrow \RR$,
$f(x)$ is said
to be concave
...
Suppose $f:\cald_f\rarrow \RR$
is a convex function. Let 
 $\alp,\beta$ be non-negative
 numbers that sum to 1,
 and $x,y, \alp x + \beta y\in \cald_f$.
Note the geometrical 
fact that (see Fig.\ref{fig-jensens})
...
a simple generalization of this fact
to sums of more than 2 terms.
Let $\{p_i\}_{i=0}^n$ be 
non-negative numbers that add to one.
Also assume that
 $\{x_i\}_{i=0}^n$, and $\sum_{=1}^n p_i x_i$ are  elements of $\cald_f$.
Then
...
\caption{The dual
function $f(x)$ of a convex function
$F(X)$
