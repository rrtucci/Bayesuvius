...
subsets, $\Sigma_{control}$ and $\Sigma_{treated}$.
All patients in 
$\Sigma_{control}$ (resp., 
...
are arguments
of $P(\cdot)$, this TPM implies that one
can swap
$\rvy_x=y_x$
and $\rvy=y_x$
inside $P(\cdot)$. For example,
\beqa
P(y_0,y_1,x)
&=&
...
Let $x, y\in \bool$ and $z\in val(\rvz)$ for
some finite, not necessarily binary set $val(\rvz)$. Define
\beq
\calp_{x', y'|x, y}=
P(\rvy_{x'}=y'|x,y)
\eeq
...
is the same thing as do-weakness
of $P(\rvy_x=y)=P(\rvy=y|\cald\rvx=x)$.
But do-weakness is a more
general concept. One can speak
of the do-weakness of $P(y_x|z)$
or of $P(y_0, y_1, a)$, etc.
In general,
any expression with
do operators is do-weak
...
\eeq
By exogeneity,
the left side of Eq.(\ref{eq-pm-with-cond})
and the left side of Eq.(\ref{eq-pm-without-cond})
are equal,
so the right sides
...
by $O$'s on the left sides.
We can also use $\eps_+=\HAT{\eps}_0=
\HAT{\eps}_1$
on the right sides to conclude that
