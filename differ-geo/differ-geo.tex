...
$(\hat{r}, \hat{\theta})$ is a non-holonomic basis because
...
$(\vece_i)^a 
(\vece_j)_a=0$ for all $i\neq j$. That basis is saidd to be normalized if $(\vece_i)^a 
(\vece_i)_a= 1$
for all $i$.
...
\begin{tabular}{|l|l|l|}
\hline
\begin{tabular}[c]{@{}l@{}}My notation for\\ basis vectors\end{tabular} & \cellcolor[HTML]{FFCCC9}not normalized & \cellcolor[HTML]{FFCCC9}\begin{tabular}[c]{@{}l@{}}normalized\\ (usually orthogonal too)\end{tabular} \\ \hline
\cellcolor[HTML]{9AFF99}\begin{tabular}[c]{@{}l@{}}coordinate\\ (holonomic) basis\end{tabular} & $\check{x}_i$ & $\hat{x}_i$ \\ \hline
\cellcolor[HTML]{9AFF99}non-holonomic basis & $\vece_i$ & $\hat{e}_i$ \\ \hline
\end{tabular}
\caption{My notation for basis vectors with various properties. A common notation is to use
...
$(\pform{\theta^1}{}, \pform{\theta^2}{} \ldots, \pform{\theta^n}{})=$ dual basis. It satisfies
...
$(\pform{\theta^1}{}, \pform{\theta^2}{})=$ dual basis. 
...
$\ul{\omega}=[\ul{\omega_{ab}}]$ must be anti-symmetric because the basis $(\hat{e}_1, \hat{e}_2)$
is orthonormal.
