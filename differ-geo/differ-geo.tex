...
\beq
(\dL{g})_e: T_e(G)\rarrow T_g(G)
,\quad
(\dL{g})_e\vecX_{e}= \AT{\frac{d\;}{d\tau}}{\tau=0} g\gamma(\tau)
\label{eq-g-e-is-ge}
\eeq
...
\beq
\vecX_g = (\dL{g})_e\vecX_e
\eeq
We extend the $\dL{g}$ map to
...
$(\dL{g})_e$ acts like left multiplication by $g$
...
\beq
\ul{\vtheta_g}: T_g(G)\rarrow T_e(G),\quad \vecX_g \mapsto (\dL{g^{-1}})_g\vecX_g = \vecX_e
\eeq
Note that $\ul{\vtheta_g}: T_g(G)\rarrow T_e(G)$
whereas $(\dL{g})_e: T_e(G)\rarrow T_g(G)$.
...
\begin{claim}
For a matrix Lie group and $g\in G$,\footnote{Thus, informally, you can think of $\ul{\vec\theta}=\dL{g^{-1}}$
