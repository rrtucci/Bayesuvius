...
\beq
T(G)=\sqcup_{g\in G}T_{g}(G)
=
\{\vecX_g(\xi)\mid g, \xi\}=\text{  tangent bundle}
\eeq

\beq
\ger{X}_L=\{\vecX(\xi)\mid \xi\}=\text{set of LIVF} 
\quad\quad\text{ where }
\vecX(\xi): G\rarrow T(G), \quad
g\mapsto \vecX_g(\xi)
\eeq
\end{mybluearea}
Then
...
\beq
\gmaurer: T_g(G)\rarrow T_e(G),\quad \vecX_g \mapsto \dL{g^{-1}}{g}\vecX_g = \vecX_e
\eeq
Note that $\gmaurer: T_g(G)\rarrow T_e(G)$
