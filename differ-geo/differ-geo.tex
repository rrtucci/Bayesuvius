...
for which differentiable functions $f(x^1, x^2, \cdots, x^n)$ exist.
 As with the basis  $\{\vec{e}_i\}_{i=0}^n$, the  t-basis $\{\vecpder{x^i}\}_{i=0}^n$ need not be orthogonal nor normalized.
...
g(\vecU, \vecV)=
g(\vecV, \vecU) = V^i U^j
\underbrace{g(\vecpder{x^i}, \vecpder{x^j})}_{\vecpder{x^i}
\cdot\vecpder{x^j}=g_{ij}}
\eeq
...
\item inner product
\beqa
\av{\vecpder{x^i}, \vecpder{x^j}}&=& g_{ij}
\\
...
\av{\vecU,\vecV}=
U^i V^j
\underbrace{g(\vecpder{x^i}, \vecpder{x^j})}_{g_{ij}}
\\
g(\ul{\alp},\vecV)&=&
\av{\ul{\alp},\vecV}=
\alp_i V^j
\underbrace{g(\ul{dx}^i, \vecpder{x^j})}_{\delta^i_j}
\\
g(\ul{\alp},\ul{\beta}) &=&
\av{\ul{\alp},\ul{\beta}}=
\alp_i \beta_j
...
\beq
|\vecpder{r}|^2 = g(\vecpder{r}, \vecpder{r})=1,
\quad
|\vecpder{\theta}|^2=
g(\vecpder{\theta}, \vecpder{\theta})=r^2,\quad
g(\vecpder{r}, \vecpder{\theta})=0
\eeq
...
\beq
g(\vecpder{x^i}, \vecpder{x^j}) = diag(1, 1, 1)
\eeq
...
\beq
g(\vecpder{x^i}, \vecpder{x^j}) = diag(-1, 1, 1, 1)
\eeq
