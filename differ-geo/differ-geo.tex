...
we will generalize 1-forms to p-forms.
For now,
just keep in mind
that we underline 
...
\mid I\in
\cali^p_<\}$
is a basis for the p-forms in $T^p_\pi(M)$. It satisfies
...
<1-form, 1-form>, and <p-form, q-form>.
...
a  p-form $\pform{\alp}{p}$
and a q-form $\pform{\beta}{q}$:
...
It's a tradeoff. Not using pullback functions or $\phi$ as in $U(S, V)= U(P, T)$ makes
the notation
...
{differ-geo/visualize-p-form.png}
\caption{You can visualize a p-form 
in $\RR^3$ as the components of flux coming out of a 1, 2 or 3 dimensional cell. }
\label{fig-visualize-p-form}
\end{figure}
...
\ul{df} = [\nabla f]^\flat
\eeq
and  for any vector field  $\vecX$,\footnote{Eq.(\ref{eq-many-words}) 
gives multiple ways to express 
the same quantity,
and this may be confusing to the beginner. As a sanity check to inure yourself to the
fact 
that all these expressions refer
to the same thing,
note that all the expressions
use 3 symbols: $X$ and $f$ 
and a symbol for differentiation,
whether it be $d$, or $\nabla$ or $\partial$ or  \qt{op}.} 
\beq
\ul{df}(\vecX) = \nabla_\vecX f 
=
\vecX\cdot\nabla f =
x^i \pder{f}{x^i}=x^i\partial_if
=\oper{\vecX} f
\label{eq-many-words}
\eeq
\end{claim}
...
Does not require a connection.
 ...
 Is only defined on p-forms.
Increases degree of p-form by 1. 

\beq
d:\wedge^p(M) \rarrow \wedge^{p+1}(M)
\eeq
...
\beq
\nnabla_\vecX: T^u_d(M)\rarrow T^u_d(M)
\eeq
...
\end{tabular}
\caption{Comparison of Cartan forms for $S^2$, assuming either the Levi-Civita or \Weit connections.}
