...
\item Two dependent and one independent variable.
...
Given a trajectory path

\beq  \gamma(t)=(u_1(t), u_2(t))
\eeq
that satisfies the system 
of ODE (ordinary differential equations) Eq.(\ref{eq-sys-ode-t}),
define

\beq
\vecX^\para = \dot{\gamma}(t)
\eeq
Then

\beq
\pback{\vecX^\para}{\dot{\gamma}(t)}\ul{\omega_\mu}=0
\eeq
So all the equations
$\ul{\omega_\mu}=0$ are satisfied by the path.
