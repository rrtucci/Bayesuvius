...
\begin{enumerate}
\item
{\bf Observing Passively. Answering \qt{What next?}:} Collecting
data
...
\item {\bf Imagining
 counterfactual situations, Analogizing. Answering \qt{What if?}:}
...
Making
predictions about
 events that have never happened (\qt{counterfactuals})
is a very Bayesian
...
We will use the
term {\bf intervention operator (or simply \qt{intervention})}
...
is as follows.\footnote{The \qt{$;5$}
in the distribution
indicates that $5$ 
...
{counterf/kappa.png}
\caption{Action of \qt{imagine} operators
...
refers to a \qt{counterfactual  event},
