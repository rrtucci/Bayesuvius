...
\beq\color{blue}
P(h(t)^\s\mid h(t-1)^\s, x(t)^\s)=
\delta(h(t)^\s,
\phi(W^{h|x}x(t)^\s + W^{h|h}h(t-1)^\s + b^h))
\eeq

\beq\color{blue}
P(Y(t)^\s\mid h(t-1)^\s)=
\delta(Y(t)^\s,
\phi(W^{y|h}h(t-1)^\s + b^y))
\eeq

\beq\color{blue}
P(y(\cdot)^\s\mid x(\cdot)^\s)\text{ = given}
\eeq

\beq\color{blue}
P(\cale(t)\mid \vecy(t), \vec{Y}(t))
=\frac{1}{nsam(\vecx)}
