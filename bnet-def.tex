...
$\rvx$
are those nodes $\rvb$
such that there is an arrow $\rvx\rarrow \rvb$.
A {\bf root node}
...
\\
\av{\rvx, \rvy}=0
&\centernot\implies& \rvx \perp \rvy
\eeqa
...
\beq
\rvx \sim Uniform(-1, 1), \quad \rvy = \rvx^2
\eeq
$\rvx$ and $\rvy$ are clearly correlated but
$\av{\rvx, \rvy}=0$, as we now show.

\beq
\av{\rvx}=0
\eeq
\beq
\av{\rvy}=\av{\rvx^2}=\frac{1}{3}
\eeq

\beq
\av{\rvx \rvy}=\av{\rvx^3}=0
\eeq
Hence,
\beq
\av{\rvx, \rvy}=
\av{\rvx \rvy}-\av{\rvx}\av{\rvy}=0
\eeq
...
$P(x|\theta)P(\theta)=x\larrow\PP \theta$
to a bad CF bnet
$P(\theta|x)P(x)=\PP x\rarrow\theta$,
where $x$ is the data
