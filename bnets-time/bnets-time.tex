...
we will say that there is
 a {\bf causal correlation}
between nodes $\rvx$ and
$\rvy$, if 
a correlation
between $\rvx$ and $\rvy$
persists
after we amputate  the
links of $\rvx$
to all its neighbors, except
$\rvy$.
A bnet has a link 
between $\rvx$ and $\rvy$ (i..e., 
an arrow 
$\rvx\rarrow\rvy$
or an arrow $\rvy\rarrow \rvx$)
iff there is a causal
correlation between $\rvx$ and $\rvy$.
$\rvx$ and $\rvy$ can
be correlated but
not causally correlated.
For example,
in the bnet $\rvx\rarrow\rvm\rarrow\rvy$,
there is no 
arrow between $\rvx$ and $\rvy$,
but $\rvx$ and $\rvy$ are correlated.
