...
\left\{
\begin{array}{l}
\dot{x}= f_1(x) + \RR y + \RR z
\\
\dot{y} = f_2(y) + \RR x + \RR z
\\
...
where parameters (i.e., $\alp_2, \alp_3, \beta_2, \beta_{\rvz}$) are positive.
...
\indi(x>K_{\rvx\rarrow\rvy})
\\
\dot{z} = -\alp_3 z + \beta_3 \indi(x> K_{\rvx\rarrow\rvz})
...
\eeq
where $\xi  >
\max(K_{\rvx\rarrow\rvy}, K_{\rvx\rarrow\rvz})$.
...
& \text{if $x$ step up, $y(0)=0$}
\\
y_0e^{-\alp_2 t}
& \text{if $x$ step down, $y(0)=y_0$}
\end{array}
\right.
\eeq
then (see Fig.\ref{fig-c1-ffl-triple})
...
&
\text{if $x$ step up,
$y(0)=0$, $y_\infty> K_{\rvy\rarrow\rvz}$}
\\
z_0 e^{-\alp_3 t}
& \text{if $x$ step down, $y(0)=y_0$, $y_\infty< K_{\rvy\rarrow\rvz}$
}
\end{array}
\right.
\eeq
where $z_\infty = \frac{\beta_3}{\alp_3}$
and $t_{lag}$ is defined so that

\beq
y(t_{lag}) = K_{\rvy\rarrow\rvz}= y_\infty(1-e^{-\alp_2 t_{lag}})
\eeq
...
y_{lag} = -\;\frac{1}{\alp_2}
\ln
\left({1- \frac{K_{\rvy\rarrow\rvz}}{y_\infty}}
\right)
\eeq
...
\\
\dot{y} =-\alp_2 y + 
\beta_2 \indi(x>K_{\rvx\rarrow\rvy})
\\
\dot{z} = -\alp_3 z + \beta_3 \indi(x> K_{\rvx\rarrow\rvz})
\indi(y<K_{\rvy\rarrow\rvz})
\end{array}
...
z=\left\{
\begin{array}{ll}
\frac{\beta_z}{\alp_3}(1- e^{-\alp_3 t})
& \text{ for } t<t_{max}
\\
y_{max}e^{-\alp_3 (t-t_{max})}
& \text{ for } t> t_{max}
\end{array}
\right.
\eeq
...
\dot{x}= f_1(x)
\\
\dot{y}= f_2(y) + \gamma_2 x
\\
\dot{z}= f_3(z) + \gamma_3 y
\end{array}
\right.
\\
\\
...
\dot{\rvx} =f_\rvx(x)
\\
\dot {\rvz_i} =f_i(x_i) + \beta_i\indi(x_i>K_i)
& \text{for $i=1,2,3$}
\end{array}
\right.
\eeq
