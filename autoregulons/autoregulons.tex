...
\right.
\eeq
where $x_\beta =\frac{\beta}{\alp}$.
As in the highpass case, $V$ has two branches: two
parabolas with the same second derivative but different vertices. 
By symmetry, their intersection is
at $x=\frac{x_\beta}{2}$,
midway between the vertices.
...
\right.
\eeq
To make $x(t)$ continuous at $t=t_K$ and have $x(t_K)=x_K$,
we must have
...
filter, 
in this case,
we can't assume $x_0=0$. 
Otherwise, $x_K=0$. Assume
$x_0>K$ instead. Then $x_K=K$. For $\alp (t-t_K) >>1$, 
\beq
