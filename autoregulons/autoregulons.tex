...
Consider
\beq
\dot{x} = -\alp x + f(x) \quad\text{ with }f(x) \approx f_0 + f_0'x
\eeq
...
\beq\frac{1}{b}d\ln(f_0 +bx)=dt
\eeq
\beq
f_0 + bx = e^{bt(t-t_0)}
\eeq

\beq x =\frac{-f_0 + e^{bt(t-t_0)}}{b} 
\eeq

Hence, $x(t)$ is a growing exponential (plus a constant) if $b>0$ and a decaying exponential otherwise. For 
negative feedback (i.e., $f'_0=\beta < 0$), $x(t)$ is always a decaying exponential,
whereas for \qt{big enough} (i.e., $-f'_0=\beta > \alp$), $x(t)$ is a growing exponential.

\subsection{Acceleration, Deceleration}
...
\caption{$(a)$ $|f(x)-\alp x|<|\beta-\alp x|$, $f(x)$ slowdown,
$(b)$ $|f(x)-\alp x|>|\beta-\alp x|$, $f(x)$ speedup}
\label{fig-xdot-versus-x}
\end{figure}
...
\caption{For single autoregulon, $\dot{x}(t)=-\alp x + f(x)$ with $f(x)=\beta\indi(x>K)$ is normal.
Speedup and slowdown
with respect to that is obtained by using more general $f(x)$. }
\label{fig-fast-normal-slow}
\end{figure}

$f(x)$ positive slope, positive autoregulation, slowdown

$f(x)$ negative slope, negative autoregulation, speedup
...
\item  $\xymatrix{\rvx\ar[r]|\ominus^K
&\rvy}$
to denote
$\rvy = \indi(\rvx< K)$
(i.e., lowpass filter)
...
\item  $\xymatrix{\rvx\ar[r]|\oplus^K
&\rvy}$
to denote
$\rvy = \indi(\rvx> K)$
(i.e., highpass filter)
...
\section{Cascades of multiple autoregulons}

\section{Negative, Positive Feedback
